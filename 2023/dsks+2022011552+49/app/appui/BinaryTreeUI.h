...
private slots:
	...
	bool inOrderPrint_search(CBiTreeNode * root, double x, double y,
	...
	void drawNode(CBiTreeNode* root, int x, int y, int depth);
	void searchChild(std::vector<CBiTreeNode*> parent);
	bool isParent(CBiTreeNode* root, CBiTreeNode* searchNode);
	void drawSearchNode(CBiTreeNode* root, int x, int y, int depth, CBiTreeNode* searchNode);
...
private:
	//计算每一层有效节点个数
	std::vector<std::pair<int, int>> nodeLevelValie;
	std::vector<std::pair<int, int>> nodeLevelInvalid;
	std::vector<std::vector<CBiTreeNode*>> nodesLevel;//记录每一层节点
	Ui::BinaryTreeUI *ui;
	Linklist *listshow;

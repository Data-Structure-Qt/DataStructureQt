...
void LinklistUI::animationDraw(int totaltime, int frametime)
{
	m_scene->insertnode(animation[totaltime][frametime].x,
		animation[totaltime][frametime].data, QLinkList.size() - 1); //链表只需要便利每个结点一次即可 
}
...
void LinklistUI::on_headinsert_btn_clicked()
{
	...
	for (int i = 0; i < QLinkList.size(); ++i)
	{
		...
		if (!ok2)
			return;
		m_scene->insertnode(i, x, QLinkList.size() - 1);   //绘制节点 
...
void LinklistUI::on_tailinsert_btn_clicked() //尾部插入按钮槽函数  
{
	...
	for (int i = 0; i < QLinkList.size(); ++i)
	{
		...
		if (!ok2)
			return;
		m_scene->insertnode(i, x, QLinkList.size() - 1);
	...
	for (int j = 0; j < QLinkList.size(); ++j) {
		...
		for (int k = 1; k <= 3; k++)//闪烁三次  
		{
			...
			for (int i = 0; i < QLinkList.size(); ++i) {
				...
				if (k % 2 == 1) {
					if (tmp != tmpj) {
						m_scene->insertnode(i, x, QLinkList.size() - 1);//向屏幕绘制  
					}
					else {
...
void LinklistUI::reverseList()
{
	...
	QList<LinklistNode> reversedList;

	// 逆置链表
	for (int i = QLinkList.size() - 1; i >= 0; --i)
	{
		reversedList.append(QLinkList.at(i));
	}

	// 更新原链表
	QLinkList = reversedList;
...
void LinklistUI::on_Invert_btn_clicked()
{
	reverseList();
}

...
public:
	...
	virtual QString createNewValue() const { return QString::number((quint64)this); }
	...
	//virtual bool setValueAttribute(const QByteArray& attrValue, const QVariant& v);
	...
	//void setValue(const QString& value) { setValueAttribute("value", value); }
	...
	void setLabelPosition(double posx, double posy);
...
QString CItem::createUniqueId(const QString& tmpl) const
{
	...
	{
		static int count = 0;
		QChar letter = QChar('A' + (count % 26)); // 从A按照 ASCII 码递增生成字母
		count++;
		return letter;  // 将字母附加到模板字符串上
	}
	...
	QChar letter;
	do {
		letter = QChar('A' + (count % 26));  // 从A按照 ASCII 码递增生成字母
		newId = letter;  // 将字母附加到模板字符串上
		count++;

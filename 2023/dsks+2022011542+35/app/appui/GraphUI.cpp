...
void GraphUI::levelTraverse(MGraph g) {
	CGraphVertexNode *node;  // 定义图节点指针变量
	queue<int> bfsQueue;  // 定义存储节点索引的队列
	vector<bool> Mvisited(g.numVertex);  // 创建布尔向量，用于记录节点是否被访问过
	vector<QString> visitedLabels;  // 记录访问过的节点标签的队列

	for (int i = 0; i < g.numVertex; i++) {  // 初始化Mvisited向量为false
		Mvisited[i] = false;
	}

	for (int i = 0; i < g.numVertex; i++) {  // 遍历图中的每个节点
		if (!Mvisited[i]) {  // 若该节点未被访问过
			Mvisited[i] = true;  // 标记节点为已访问
			visitedLabels.push_back(QString::fromStdString(g.adjointList[i].data));  // 记录访问过的节点标签
			qDebug() << QString::fromStdString(g.adjointList[i].data) << " ";  // 打印节点标签
			NodeChangeColor(QString::fromStdString(g.adjointList[i].data));  // 改变节点颜色
			animationRecord(1, QString::fromStdString(g.adjointList[i].data), NULL);  // 记录动画
			m_quickHelp->level_code(1);
			bfsQueue.push(i);  // 将节点入队
			while (!bfsQueue.empty()) {  // 当队列非空时，执行以下操作
				int queueValue;  // 定义队列头部的节点索引
				queueValue = bfsQueue.front();  // 取出队首的值
				bfsQueue.pop();  // 队首元素出队
				node = g.adjointList[queueValue].firstEdage;  // 获取队首节点的邻接节点
				while (node) {  // 遍历队首节点的邻接节点
					if (!Mvisited[node->adjointVertex]) {  // 若邻接节点未被访问过
						Mvisited[node->adjointVertex] = true;  // 标记邻接节点为已访问
						visitedLabels.push_back(QString::fromStdString(g.adjointList[node->adjointVertex].data));  // 记录访问过的节点标签
						m_quickHelp->level_code(2);
						bfsQueue.push(node->adjointVertex);  // 将邻接节点入队
						EdgeChangeColor(QString::fromStdString(g.adjointList[queueValue].data),  // 改变边颜色
							QString::fromStdString(g.adjointList[node->adjointVertex].data));
						animationRecord(2,  // 记录动画
							QString::fromStdString(g.adjointList[queueValue].data),
							QString::fromStdString(g.adjointList[node->adjointVertex].data));
						m_quickHelp->level_code(3);
						NodeChangeColor(QString::fromStdString(g.adjointList[node->adjointVertex].data));  // 改变节点颜色
						animationRecord(1,  // 记录动画
							QString::fromStdString(g.adjointList[node->adjointVertex].data), NULL);
						qDebug() <<  // 打印节点标签
							QString::fromStdString(g.adjointList[node->adjointVertex].data) << " ";
					}
					node = node->next;  // 处理下一个邻接节点
				}
			}
		}
	}

	// 使用QMessageBox输出访问过的节点标签
	QString visitedNodeLabelString = QString::fromLocal8Bit("层序遍历结果为: ");
	for (const QString& label : visitedLabels) {
		visitedNodeLabelString += label + " ";
	}
	QMessageBox::information(this, QString::fromLocal8Bit("结果"), visitedNodeLabelString);
}
//改变节点的颜色

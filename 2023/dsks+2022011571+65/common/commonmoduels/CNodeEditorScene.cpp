...
void CNodeEditorScene::initialize()
{
	...
	CAttribute nodeAttr("color", "Color", QColor(100, 149, 237), ATTR_FIXED);
...
void CNodeEditorScene::setEditMode(EditMode mode)
{
	if (m_editMode != mode)
	{
		...
		{
		case EM_Transform:
			getCurrentView()->setDragMode(QGraphicsView::RubberBandDrag); //0217
...
void CNodeEditorScene::insertblock(int location, int data) {
	if (location == 0) {
		orgin.setX(-50);
		orgin.setY(-400);
		flag = false;
	...
	qreal tableX = orgin.x() - (numCols * cellWidth) / 2;
	qreal tableY = orgin.y() + 50;

	for (int row = 0; row < numRows; ++row) {
		for (int col = 0; col < numCols; ++col) {
			QRectF rect(tableX + col * cellWidth, tableY + row * cellHeight, cellWidth, cellHeight);
			QGraphicsRectItem *tableCell = addRect(rect, QPen(Qt::black), QBrush(Qt::white));
			...
			if (row == 0) {
				// 第一行    // 21，39，48，49
				if (col == 0) {
					blockIdentifier = "Max key word";
				}
				else if (col == 1) {
					blockIdentifier = QString::number(21);
				}
				else if (col == 2) {
					blockIdentifier = QString::number(39);
				}
				else if (col == 3) {
					blockIdentifier = QString::number(48);
				}
				else if (col == 4) {
					blockIdentifier = QString::number(49);
				}
			}
			else if (row == 1) {
				// 第二行
				if (col == 0) {
					blockIdentifier = "Start address";
				}
				else {
					// 1，5，9，13
					blockIdentifier = QString::number(col * 4 - 3);
				}
			}

			QGraphicsTextItem *textItem = new QGraphicsTextItem(blockIdentifier, tableCell);
			textItem->setPos(rect.x() + 5, rect.y() + 5);  // 设置文本位置
		}
	}

	// Update orgin for the next set of blocks
	orgin.setY(tableY + numRows * cellHeight); // 移动 orgin 到下一行

   // 添加一个新的一行两列的表格
	int newRow = numRows + 1;  // 新行数
	int newCols = 2;           // 新列数

	// 设置新表格的起始位置并向左偏移一些
	qreal xOffset = 140;  // 左偏移量
	qreal yOffset = 80;  // 向上偏移量
	qreal newTableX = orgin.x() - (newCols * cellWidth) / 2 - xOffset;
	qreal newTableY = orgin.y() + (newRow - 1) * cellHeight - yOffset; // 向上偏移

	// 设置新的单元格宽度为原来的两倍
	qreal newCellWidth = cellWidth * 2;  // 原来宽度的两倍

	for (int col = 0; col < newCols; ++col) {
		QRectF rect(newTableX + col * newCellWidth, newTableY, newCellWidth, cellHeight);  // 使用两倍宽度的单元格
		QGraphicsRectItem *tableCell = addRect(rect, QPen(Qt::black), QBrush(Qt::white));
		...
		// 根据列数决定文本内容
		if (col == 0) {
			blockIdentifier = "Element position";
		}
		else if (col == 1) {
			blockIdentifier = "1-4";
		}
...
void CNodeEditorScene::insertfindnode(int location, int data)
{
	...
	QPolygonF pf;
	pf << QPointF(px - 50, py - 12) << QPointF(px - 50, py + 12) << QPointF(px - 25, py);
	pItem->setPolygon(pf);
	addItem(pItem);
...
void CNodeEditorScene::onLeftDoubleClick(QGraphicsSceneMouseEvent* mouseEvent, QGraphicsItem* clickedItem)
{
	...
	if (!clickedItem)
	{
		...
		addUndoState();
		return;
		//myPix = new QPixmap(1000,1000); //将自定义的绘制版大小设置为画板大小
...
//屏幕绘制函数
void  CNodeEditorScene::pixmapScreen()
{

}

void CNodeEditorScene::mousePressEvent(QGraphicsSceneMouseEvent* e) //鼠标按下事件
{
	// 0220 画板限制
	if (e->scenePos().x() < sceneRect().left() || e->scenePos().x() > sceneRect().topRight().x())
	{
		return;
	}
	if (e->scenePos().y() < sceneRect().top() || e->scenePos().y() > sceneRect().bottom())
	{
		return;
	}
...
void CNodeEditorScene::mouseMoveEvent(QGraphicsSceneMouseEvent* e) //鼠标移动事件
{
	//申请一个画家类
	...
	//this->addLine(startPoint.x(), startPoint.y(), e->scenePos().x(), e->scenePos().y(), pen);
	//开始画线
	//painter.drawLine(startPoint, e->screenPos());
...
void CNodeEditorScene::paintEvent(QPaintEvent*) //绘制事件
{
	////1.请一个画家，依附于this画板

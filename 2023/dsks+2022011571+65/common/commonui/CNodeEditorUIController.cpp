...
CNodeEditorUIController::CNodeEditorUIController(CMainWindow *parent)
  ...
  // connect scene
  connect(m_editorScene, &CEditorScene::sceneChanged, parent,
          &CMainWindow::onDocumentChanged);
  connect(m_editorScene, &CEditorScene::sceneChanged, this,
          &CNodeEditorUIController::onSceneChanged);
  connect(m_editorScene, &CEditorScene::selectionChanged, this,
          &CNodeEditorUIController::onSelectionChanged);

  connect(m_editorScene, &CEditorScene::infoStatusChanged, this,
          &CNodeEditorUIController::onSceneStatusChanged);
  connect(m_editorScene, &CNodeEditorScene::editModeChanged, this,
          &CNodeEditorUIController::onEditModeChanged);

  connect(m_editorScene, &CEditorScene::sceneDoubleClicked, this,
          &CNodeEditorUIController::onSceneDoubleClicked);
  ...
  connect(m_ogdfController, SIGNAL(layoutFinished()), this,
          SLOT(onLayoutFinished()));
#endif

  // GraphViz
#ifdef USE_GVGRAPH
  m_gvController = new CGVGraphLayoutUIController(parent, m_editorScene);
  connect(m_gvController, SIGNAL(layoutFinished()), this,
          SLOT(onLayoutFinished()));

#ifdef Q_OS_WIN32
  QString pathToGraphviz =
      QCoreApplication::applicationDirPath() + "/../tools/graphviz";
  ...
  connect(&m_backupTimer, &QTimer::timeout, this,
          &CNodeEditorUIController::doBackup);
}
...
bool CNodeEditorUIController::loadFromFile(const QString &format,
                                           const QString &fileName,
                                           QString *lastError) {
  if (m_ioController)
    return m_ioController->loadFromFile(format, fileName, *m_editorScene,
                                        lastError);
  else
    return false;
}

bool CNodeEditorUIController::saveToFile(const QString &format,
                                         const QString &fileName,
                                         QString *lastError) {
  if (m_ioController)
    return m_ioController->saveToFile(format, fileName, *m_editorScene,
                                      lastError);
  else
    return false;
}
...
void CNodeEditorUIController::exportFile() {
  m_ioController->exportImage(*m_editorScene);
}

void CNodeEditorUIController::exportPDF() {
  m_ioController->exportPDF(*m_editorScene);
}

void CNodeEditorUIController::exportSVG() {
  m_ioController->exportSVG(*m_editorScene);
}

void CNodeEditorUIController::exportDOT() {
  m_ioController->exportDOT(*m_editorScene);
}

bool CNodeEditorUIController::importCSV(const QString &fileName,
                                        QString *lastError) {
...
void CNodeEditorUIController::createMenus() {
  ...
  connect(exportAction, &QAction::triggered, this,
          &CNodeEditorUIController::exportFile);

  QAction *exportActionSVG =
      new QAction(QIcon(":/Icons/SVG"), tr("Export to &SVG..."));
  m_parent->getFileMenu()->insertAction(exportAction, exportActionSVG);
  connect(exportActionSVG, &QAction::triggered, this,
          &CNodeEditorUIController::exportSVG);

  QAction *exportActionPDF =
      new QAction(QIcon(":/Icons/PDF"), tr("Export to &PDF..."));
  m_parent->getFileMenu()->insertAction(exportActionSVG, exportActionPDF);
  connect(exportActionPDF, &QAction::triggered, this,
          &CNodeEditorUIController::exportPDF);

  QAction *exportActionDOT =
      new QAction(QIcon(":/Icons/DOT"), tr("Export to &DOT/GraphViz..."));
  m_parent->getFileMenu()->insertAction(exportActionPDF, exportActionDOT);
  connect(exportActionDOT, &QAction::triggered, this,
          &CNodeEditorUIController::exportDOT);
  ...
  QAction *undoAction = editMenu->addAction(QIcon(":/Icons/Undo"),
  ...
  connect(undoAction, &QAction::triggered, this,
          &CNodeEditorUIController::undo);
  connect(m_editorScene, &CEditorScene::undoAvailable, undoAction,
          &QAction::setEnabled);
  undoAction->setEnabled(m_editorScene->availableUndoCount());

  QAction *redoAction = editMenu->addAction(QIcon(":/Icons/Redo"),
                                            QString::fromLocal8Bit("重做"));
  redoAction->setStatusTip(QString::fromLocal8Bit("重做最近的编辑"));
  redoAction->setShortcut(QKeySequence::Redo);
  connect(redoAction, &QAction::triggered, this,
          &CNodeEditorUIController::redo);
  connect(m_editorScene, &CEditorScene::redoAvailable, redoAction,
          &QAction::setEnabled);
  ...
  QAction *selAction =
      editMenu->addAction(QIcon(":/Icons/SelectAll"), tr("Select All"));
  ...
  connect(m_editModesGroup, &QActionGroup::triggered, this,
          &CNodeEditorUIController::sceneEditMode);

  //   modeDefaultAction = editMenu->addAction(QIcon(":/Icons/Mode-Select"),
  //   QString::fromLocal8Bit("选择")); modeDefaultAction->setToolTip(tr("Items
  //   selection mode")); modeDefaultAction->setStatusTip(tr("Select/deselect
  //   items in the document")); modeDefaultAction->setCheckable(true);
  //   modeDefaultAction->setActionGroup(m_editModesGroup);
  //   modeDefaultAction->setChecked(m_editorScene->getEditMode() ==
  //   EM_Default); modeDefaultAction->setData(EM_Default);

  //   modeNodesAction = editMenu->addAction(QIcon(":/Icons/Mode-AddNodes"),
  //   tr("Create Nodes")); modeNodesAction->setToolTip(tr("Adding new nodes
  //   mode")); modeNodesAction->setStatusTip(tr("Quickly add nodes & edges"));
  ...
  // modeTransformAction = editMenu->addAction(QIcon(":/Icons/Mode-Transform"),
  // QString::fromLocal8Bit("移动")); modeTransformAction->setToolTip(tr("Items
  // transformation mode")); modeTransformAction->setStatusTip(tr("Transform
  // selected nodes (scale & move)")); modeTransformAction->setCheckable(true);
  // modeTransformAction->setActionGroup(m_editModesGroup);
  // modeTransformAction->setChecked(m_editorScene->getEditMode() ==
  // EM_Transform); modeTransformAction->setData(EM_Transform);

  // modeFactorAction = editMenu->addAction(QIcon(":/Icons/Mode-Factor"),
  // tr("Factor")); modeFactorAction->setToolTip(tr("Positions transformation
  // mode")); modeFactorAction->setStatusTip(tr("Scale position of selected nodes
  // (move only)")); modeFactorAction->setCheckable(true);
  ...
  //   modeTreeAction = editMenu->addAction(QIcon(":/Icons/Icons/addtree"),
  //   tr("Create Tree")); modeTreeAction->setToolTip(tr("Create Binaray Tree by
  //   Input")); modeTreeAction->setStatusTip(tr("Scale position of selected
  //   nodes (move only)")); modeTreeAction->setCheckable(true);
  ...
  QAction *sceneCropAction =
      editMenu->addAction(QIcon(":/Icons/Crop"), tr("&Crop Area"));
  ...
  QAction *schemesAction =
      editMenu->addMenu(m_schemesController->getSchemesMenu());
  schemesAction->setText(QString::fromLocal8Bit("应用颜色"));
  schemesAction->setStatusTip(
      QString::fromLocal8Bit("将预定义的配色方案应用到文档"));
  ...
  gridAction =
      m_viewMenu->addAction(QIcon(":/Icons/Grid-Show"), tr("Show &Grid"));
  ...
  gridSnapAction =
      m_viewMenu->addAction(QIcon(":/Icons/Grid-Snap"), tr("&Snap to Grid"));
  ...
  m_actionShowNodeIds->setChecked(
      m_editorScene->isClassAttributeVisible(class_node, attr_id));
  connect(m_actionShowNodeIds, SIGNAL(toggled(bool)), this,
          SLOT(showNodeIds(bool)));
  ...
  m_actionShowEdgeIds->setChecked(
      m_editorScene->isClassAttributeVisible(class_edge, attr_id));
  connect(m_actionShowEdgeIds, SIGNAL(toggled(bool)), this,
          SLOT(showEdgeIds(bool)));

  // m_viewMenu->addSeparator();

  /* zoomAction = m_viewMenu->addAction(QIcon(":/Icons/ZoomIn"),
   QString::fromLocal8Bit("放大")); zoomAction->setStatusTip(tr("Zoom view
   in")); zoomAction->setShortcut(QKeySequence::ZoomIn); connect(zoomAction,
   &QAction::triggered, this, &CNodeEditorUIController::zoom);

   unzoomAction = m_viewMenu->addAction(QIcon(":/Icons/ZoomOut"),
   QString::fromLocal8Bit("缩小")); unzoomAction->setStatusTip(tr("Zoom view
   out")); unzoomAction->setShortcut(QKeySequence::ZoomOut);
   connect(unzoomAction, &QAction::triggered, this,
   &CNodeEditorUIController::unzoom);

   resetZoomAction = m_viewMenu->addAction(QIcon(":/Icons/ZoomReset"),
   tr("&Reset Zoom")); resetZoomAction->setStatusTip(tr("Zoom view to 100%"));
   connect(resetZoomAction, &QAction::triggered, this,
   &CNodeEditorUIController::resetZoom);

   fitZoomAction = m_viewMenu->addAction(QIcon(":/Icons/ZoomFit"),
   QString::fromLocal8Bit("满窗显示")); fitZoomAction->setStatusTip(tr("Zoom to
   fit all the items to view")); connect(fitZoomAction, &QAction::triggered,
   m_editorView, &CEditorView::fitToView);

   fitZoomSelectedAction =
   m_viewMenu->addAction(QIcon(":/Icons/ZoomFitSelected"),
   QString::fromLocal8Bit("选中调整"));
   fitZoomSelectedAction->setToolTip(tr("Fit selected items to view"));
   fitZoomSelectedAction->setStatusTip(tr("Zoom to fit selected items to
   view")); connect(fitZoomSelectedAction, &QAction::triggered, m_editorView,
   &CEditorView::fitSelectedToView);

   fitZoomBackAction = m_viewMenu->addAction(QIcon(":/Icons/ZoomFitBack"),
   QString::fromLocal8Bit("大小回退")); fitZoomBackAction->setStatusTip(tr("Zoom
   to previous state before last fit")); connect(fitZoomBackAction,
   &QAction::triggered, m_editorView, &CEditorView::zoomBack);*/
  ...
  //   connect(resetZoomAction2, &QAction::triggered, this,
  //   &CNodeEditorUIController::resetZoom);
  ...
  goToBeginAction = m_viewMenu->addAction(
      QIcon(":/Icons/gotobegin"), QString::fromLocal8Bit("跳到开始")); //重置
  goToBeginAction->setShortcut(QKeySequence::ZoomIn);
  connect(goToBeginAction, &QAction::triggered, this,
          &CNodeEditorUIController::onClickGoToBeginButton);

  backAction = m_viewMenu->addAction(
      QIcon(":/Icons/back"),
      QString::fromLocal8Bit("后退一步")); //从开始运行到上一步
  backAction->setShortcut(QKeySequence::ZoomOut);
  connect(backAction, &QAction::triggered, this,
          &CNodeEditorUIController::onClickStepBackButton);

  resetZoomAction = m_viewMenu->addAction(
      QIcon(":/Icons/play"),
      QString::fromLocal8Bit("开始")); //开始|暂停 运行 时间间隔为默认值
  connect(resetZoomAction, &QAction::triggered, this,
          &CNodeEditorUIController::onClickPlayButton);

  fitZoomSelectedAction = m_viewMenu->addAction(
      QIcon(":/Icons/go"), QString::fromLocal8Bit("前进一步")); //向下运行一步
  connect(fitZoomSelectedAction, &QAction::triggered, this,
          &CNodeEditorUIController::onClickStepForwardButton);

  fitZoomBackAction =
      m_viewMenu->addAction(QIcon(":/Icons/gotoend"),
                            QString::fromLocal8Bit("跳到结束")); //运行到结束
  connect(fitZoomBackAction, &QAction::triggered, this,
          &CNodeEditorUIController::onClickGoToEndButton);
...
void CNodeEditorUIController::createPanels() {
  ...
  //	"defaultsDock", QString::fromLocal8Bit("默认属性"),
  //Qt::LeftDockWidgetArea, 	m_defaultsPanel = new
  //CClassAttributesEditorUI(m_parent)
  //);
  ...
  // properties
  /* m_parent->createDockWindow(
               "propertyDock", QString::fromLocal8Bit("项目属性"),
     Qt::RightDockWidgetArea, m_propertiesPanel = new
     CNodeEdgePropertiesUI(m_parent)
       );
  ...
  /* connections
  m_parent->createDockWindow(
          "connectionsDock", QString::fromLocal8Bit("拓扑结构"),
  Qt::LeftDockWidgetArea, m_connectionsPanel = new CCommutationTable(m_parent)
  );
  ...
  */
  ...
  /*auto *linklistDock = m_parent->createDockWindow(
          ...
          m_linklistPanel = new LinklistUI(m_parent)
  );*/
  m_parent->createDockWindow("linklistDock", QString::fromLocal8Bit("控制面板"),
  ...
  auto *quickHelpDock = m_parent->createDockWindow(
      "quickHelpDock", QString::fromLocal8Bit("代码追踪"),
      Qt::RightDockWidgetArea, m_quickHelpPanel = new CQuickHelpUI(m_parent));
  ...
  m_parent->getHelpMenu()->insertAction(
      m_parent->getHelpMenu()->actions().first(), quickHelpAction);
}
...
void CNodeEditorUIController::onSceneChanged() {
  ...
  m_statusLabel->setText(
      tr("Nodes: %1 | Edges: %2").arg(nodes.size()).arg(edges.size()));
...
void CNodeEditorUIController::onSceneStatusChanged(int status) {
    ...
    if (isAddNodesMode)
      ...
      onSceneHint(
          tr("Ctrl + Click - (un)select item | Click & drag or Ctrl/Shift + "
             ...
             "selected items | Double click - edit item's label"));
    return;
  ...
  default:
    if (isAddNodesMode)
      onSceneHint(
          tr("Click - create new node | Click & drag - create new connection") +
          arrowMoveHint);
    else
...
void CNodeEditorUIController::onSceneDoubleClicked(
...
void CNodeEditorUIController::doBackup() {
  ...
  if (backupFileName.isEmpty()) {
    m_parent->statusBar()->showMessage(tr("Cannot backup non-saved document"),
                                       2000);
    return;
  } else {
  ...
  m_parent->statusBar()->showMessage(
      tr("Running backup... (%1)").arg(backupFileName));
  qApp->processEvents();

  CFileSerializerXGR writer;
  if (writer.save(backupFileName, *m_editorScene)) {
    m_parent->statusBar()->showMessage(
        tr("Backup done (%1)").arg(backupFileName), 2000);
  } else {
    m_parent->statusBar()->showMessage(
        tr("Backup failed (%1)").arg(backupFileName), 2000);
  }
}
...
void CNodeEditorUIController::onNewDocumentCreated() {
  ...
  m_editorScene->createClassAttribute("",
                                      QString::fromLocal8Bit("注解").toLatin1(),
                                      "Comment", QString(), ATTR_NONE);
...
void CNodeEditorUIController::doReadSettings(QSettings &settings) {
  ...
  m_optionsData.backupPeriod =
      settings.value("backupPeriod", m_optionsData.backupPeriod).toInt();

  settings.beginGroup("GraphViz");
  m_optionsData.graphvizPath =
      settings.value("path", m_optionsData.graphvizPath).toString();
  m_optionsData.graphvizDefaultEngine =
      settings.value("defaultEngine", m_optionsData.graphvizDefaultEngine)
          .toString();
  settings.endGroup();
...
void CNodeEditorUIController::readDefaultSceneSettings() {
  ...
  QColor bgColor =
      settings.value("background", m_editorScene->backgroundBrush().color())
          .value<QColor>();
  QPen gridPen =
      settings.value("grid.color", m_editorScene->getGridPen()).value<QPen>();
  int gridSize =
      settings.value("grid.size", m_editorScene->getGridSize()).toInt();
  bool gridEnabled =
      settings.value("grid.enabled", m_editorScene->gridEnabled()).toBool();
  bool gridSnap =
      settings.value("grid.snap", m_editorScene->gridSnapEnabled()).toBool();
...
#ifdef USE_GVGRAPH
  connect(&dialog, &CSceneOptionsDialog::testGraphviz, m_gvController,
          &CGVGraphLayoutUIController::runGraphvizTest);
#endif
...
void CNodeEditorUIController::updateFromActions() {
  if (m_editorScene) {
    m_editorScene->setClassAttributeVisible(class_node, attr_id,
                                            m_actionShowNodeIds->isChecked());
    m_editorScene->setClassAttributeVisible(class_edge, attr_id,
...
void CNodeEditorUIController::zoom() { m_editorView->zoomBy(1.3); }

void CNodeEditorUIController::unzoom() { m_editorView->zoomBy(1.0 / 1.3); }

void CNodeEditorUIController::resetZoom() { m_editorView->zoomTo(1.0); }
...
void CNodeEditorUIController::editNodePort() {
  CNodePort *port =
      dynamic_cast<CNodePort *>(m_editorScene->getContextMenuTrigger());
  if (port)
    editNodePort(*port);
}
...
void CNodeEditorUIController::find() { m_searchDialog->exec(*m_editorScene); }
...
void CNodeEditorUIController::changeItemId() {
  auto sceneActions =
      dynamic_cast<CNodeSceneActions *>(m_editorScene->getActions());
...
void CNodeEditorUIController::onClickPlayButton() {
  ...
  {
    playButtonFlag = 0;
    // resetZoomAction->setText(QString::fromLocal8Bit("暂停"));
    // //点击后进入此处变为待暂停
    // resetZoomAction->setIcon(QIcon(":/Icons/suspend"));
    // //按钮变为暂停按钮

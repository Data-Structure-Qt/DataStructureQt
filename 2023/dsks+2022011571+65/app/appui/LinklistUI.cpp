...
LinklistUI::LinklistUI(QWidget *parent)
    : QWidget(parent), ui(new Ui::LinklistUI), m_scene(NULL) {
  ui->setupUi(this);
}
...
void LinklistUI::suspendButton() { endplaybuttonflag = 1; }
...
void LinklistUI::animationSearchRecord(int totaltime, int frame, int x,
                                       int data, int flag, int codeid) {
...
void LinklistUI::animationDraw(int totaltime, int frametime) {
  m_scene->insertnode(
      animation[totaltime][frametime].x,

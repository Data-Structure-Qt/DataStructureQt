...
CNodeEditorScene::CNodeEditorScene(QObject* parent)
    : Super(parent), m_editMode(EM_Default), m_startNode(NULL), m_endNode(NULL),
    m_connection(NULL), m_realStart(false), m_state(IS_None) {
    // default factories
...
bool CNodeEditorScene::fromGraph(const Graph& g) {
    ...
    for (const Node& n : g.nodes) {
        ...
        for (auto it = n.ports.constBegin(); it != n.ports.constEnd(); ++it) {
            CNodePort* port = node->addPort(it.key().toLatin1(), it.value().anchor,
...
bool CNodeEditorScene::toGraph(Graph& g) {
    ...
    static AttrInfo _vis_(
        { attr_labels_visIds, "Visible Labels", QVariant::StringList });
...
void CNodeEditorScene::initialize() {
    ...
    createClassAttribute("node", "size", "Size", QSizeF(48.0, 48.0),
    ...
    // ATTR_MAPPED);
    createClassAttribute("node", "x", "X-Coordinate", 0.0f,
        ATTR_NODEFAULT | ATTR_MAPPED | ATTR_FIXED);
    createClassAttribute("node", "y", "Y-Coordinate", 0.0f,
        ATTR_NODEFAULT | ATTR_MAPPED | ATTR_FIXED);
    ...
    createClassAttribute("node", "stroke.color", "Stroke Color",
        QColor(Qt::black), ATTR_FIXED);

    createClassAttribute("node", "degree", "Degree", 0,
...
void CNodeEditorScene::onLeftButtonPressed(
    QGraphicsSceneMouseEvent* mouseEvent) {
    Super::onLeftButtonPressed(mouseEvent);

    // add nodes?
    if (m_editMode == EM_AddNodes ||
        isItemAt<CNodePort>(mouseEvent->scenePos())) {
        deselectAll();
...
bool CNodeEditorScene::onClickDrag(QGraphicsSceneMouseEvent* mouseEvent,
...
bool CNodeEditorScene::onDoubleClickDrag(QGraphicsSceneMouseEvent* mouseEvent,
    const QPointF& clickPos) {
    // debug
    if (clickPos == QPointF())
        qDebug() << "bug";
...
void CNodeEditorScene::onDropped(QGraphicsSceneMouseEvent* mouseEvent,
...
void CNodeEditorScene::onLeftClick(QGraphicsSceneMouseEvent* mouseEvent,
...
void CNodeEditorScene::onLeftDoubleClick(QGraphicsSceneMouseEvent* mouseEvent,
    QGraphicsItem* clickedItem) {
    // clicked on empty space?
    if (!clickedItem) {
        // create a node here
...
QList<QGraphicsItem*> CNodeEditorScene::getCopyPasteItems() const {
    ...
    for (auto item : selectedItems()) {
        ...
        else if (auto node = dynamic_cast<CNode*>(item)) {
            // orphaned nodes only
...
bool CNodeEditorScene::doUpdateCursorState(Qt::KeyboardModifiers keys,
...
void CNodeEditorScene::dataDraw(double x, double y, double w, double h,
    int arr) {
...
void CNodeEditorScene::drawCircle(int x, int y, int data, int p_x,
    int p_y) //绘制圆圈+箭头
{
    ...
    if (data != -1) {
        ...
        pen.setColor(Qt::green);    //外边框的颜色
        rectItem->setPen(pen);
        rectItem->setBrush(QBrush(QColor(255, 255, 255))); //填充颜色
...
void CNodeEditorScene::drawCircleWithChar(int x, int y, int data, int p_x,
    int p_y) //绘制圆圈+箭头
{
    QGraphicsLineItem* lineItem = new QGraphicsLineItem();
    QPen pen2 = lineItem->pen();          //画笔 绘制外框
    ...
    if (data != -1) {
        ...
        QGraphicsTextItem* pItem = new QGraphicsTextItem();
        pItem->setPlainText(
            QString::fromStdString(std::string(1, char(data)))); // 纯文本
        pItem->setDefaultTextColor(QColor(0, 0, 0));             // 文本色
        pItem->setPos(QPointF(x + 7, y + 5));                    //位置
        QFont font = pItem->font();
        font.setPixelSize(12); // 像素大小
...
void CNodeEditorScene::drawCircle_search(int x, int y, int data, int p_x,
    int p_y,
    int data1) //绘制圆圈+箭头 颜色
{
    QGraphicsLineItem* lineItem = new QGraphicsLineItem();
    QPen pen2 = lineItem->pen();       //画笔 绘制外框
...
void CNodeEditorScene::drawCircle2(int x, int y, int data1, int data2, int p_x,
    int p_y) //绘制圆圈+箭头
{
    QGraphicsLineItem* lineItem = new QGraphicsLineItem();
    QPen pen2 = lineItem->pen();          //画笔 绘制外框
    ...
    QGraphicsTextItem* pItem = new QGraphicsTextItem();
    pItem->setPlainText(
        QString::fromStdString(std::string(1, char(data1))) + ',' +
        QString::fromStdString(std::string(1, char(data2)))); // 纯文本
    pItem->setDefaultTextColor(QColor(0, 0, 0));              // 文本色
    pItem->setPos(QPointF(x + 7, y + 5));                     //位置
    QFont font = pItem->font();
    font.setPixelSize(10); // 像素大小
...
void CNodeEditorScene::drawCircle2WithChar(int x, int y, int data1, int data2,
    int p_x,
    int p_y) //绘制圆圈+箭头
{
    QGraphicsLineItem* lineItem = new QGraphicsLineItem();
    ...
    QGraphicsTextItem* pItem = new QGraphicsTextItem();
    pItem->setPlainText(QString::number(data1) + ',' +
        QString::number(data2)); // 纯文本
    pItem->setDefaultTextColor(QColor(0, 0, 0)); // 文本色
    pItem->setPos(QPointF(x + 7, y + 5));        //位置
    QFont font = pItem->font();
    font.setPixelSize(10); // 像素大小
...
void CNodeEditorScene::drawCircle_simulate(int x, int y, int data, int p_x,
    int p_y) //模拟绘制圆圈+箭头
{
    QGraphicsEllipseItem* rectItem = new QGraphicsEllipseItem; //绘制圆形
    QPen pen = rectItem->pen(); //画笔 绘制外框
...
void CNodeEditorScene::clearScreen() {
    QRectF Rect(-2000, -2000, 4000,
        4000); //删除-1000 -1000 1000 1000 方块内的所有图形

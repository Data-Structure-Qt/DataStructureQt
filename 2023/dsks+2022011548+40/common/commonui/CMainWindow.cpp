...
void CMainWindow::createMainMenu()
{
    ...
    m_newDocument = m_fileMenu->addAction(QIcon(":/Icons/New"), QString::fromLocal8Bit("新建"));
    m_newDocument->setStatusTip(QString::fromLocal8Bit("创建新文件"));

    fillNewFileMenu();

    m_openDocument = m_fileMenu->addAction(QIcon(":/Icons/Open"), QString::fromLocal8Bit("打开..."), this, SLOT(on_actionOpen_triggered()));
    m_openDocument->setStatusTip(QString::fromLocal8Bit("打开一个文件"));
    m_openDocument->setShortcut(QKeySequence::Open);

    m_saveDocument = m_fileMenu->addAction(QIcon(":/Icons/Save"), QString::fromLocal8Bit("保存"), this, SLOT(on_actionSave_triggered()));
	...
	m_exportDocument->setStatusTip(QStringLiteral("将当前文档导出为其他格式"));
...
void CMainWindow::on_actionOpen_triggered()
{
	...
	QString fileName = QFileDialog::getOpenFileName(NULL, title,
		loadName, filter, &m_lastOpenFilter);
	if (fileName.isEmpty())
		return;
...
bool CMainWindow::doOpenDocument(const QString &fileName)
{
    ...
    {
		statusBar()->showMessage(QStringLiteral("打开失败: %1").arg(fileName));

        QMessageBox::critical(NULL, 
			fileName,
			QStringLiteral("文档文件不存在或不可访问")
		);
    ...
    if (openDocument(normalizedName, fileDocType))
    {
        ...
        statusBar()->showMessage(QStringLiteral("已成功打开: %1").arg(fileName));
	...
	QMessageBox::critical(NULL,
		fileName,
		QStringLiteral("无法打开文档。检查访问权限和路径."));
...
void CMainWindow::onOpenDocumentDialog(QString &title, QString &filter)
{
    ...
    {
        allFilters.chop(1);
        allFilters = QStringLiteral("任何支持的格式 (%1)").arg(allFilters);
        filter += allFilters;
...
bool CMainWindow::doSaveDocument(const QString &fileName, const QString &selectedFilter, const QByteArray &docType)
{
    if (saveDocument(fileName, selectedFilter, docType))
    {
        ...
        statusBar()->showMessage(QStringLiteral("文档保存成功."));
    ...
    {
        QMessageBox::critical(NULL, tr("Save Error"), QStringLiteral("无法保存文档，检查访问权限和路径。"));
    }

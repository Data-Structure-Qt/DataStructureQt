...
void CNodeEditorScene::initialize()
{
	...
	// default node attr
	CAttribute nodeAttr("color", "Color", QColor("#00FFFF"), ATTR_FIXED);//初始为蓝色
	setClassAttribute("node", nodeAttr);

	CAttribute shapeAttr("shape", "Shape", "disc", ATTR_FIXED);
	setClassAttribute("node", shapeAttr);

	createClassAttribute("node", "size", "Size", QSizeF(60.0, 60.0), ATTR_MAPPED | ATTR_FIXED);
	...
	// default edge attr
	CAttribute edgeAttr("color", "Color", QColor(Qt::black), ATTR_FIXED);

...
void CNodeEditorScene::insertnode(int x, int y, int data)
{
	orgin = { x * 150.0,y * 75.0 - this->width() / 2 };
	auto right = createNewNode(getSnapped(orgin));//绘制矩形
	if (data == -1)
	{
		return;
	}
	...
	{
		onActionEditLabel(right, data);
		QPointF addnext(50, 0);
		auto left = createNewNode(getSnapped(orgin - addnext));//绘制矩形
		QGraphicsItem* hoverItem = getItemAt(getSnapped(orgin - addnext - addnext - addnext));
		CNode* item = dynamic_cast<CNode*>(hoverItem);
		createNewConnection(item, left);//绘制箭头

	}
	//qDebug("%d", data);
	//QPointF addnext(100, 0);
	//orgin = { x * 150.0,y * 100.0 };
	//auto node = createNewNode(getSnapped(orgin - addnext));//绘制矩形

...
void CNodeEditorScene::addstack()
{
	QGraphicsLineItem* lineitem1 = new QGraphicsLineItem;
	lineitem1->setLine(-30, 0, -30, 230);//添加栈的左边
	QPen penA = QPen();
	penA.setWidth(2);
	lineitem1->setPen(penA);
	addItem(lineitem1);

	QGraphicsLineItem* lineitem2 = new QGraphicsLineItem;
	lineitem2->setLine(30, 0, 30, 230);//添加栈的左边
	lineitem2->setPen(penA);
	addItem(lineitem2);

	QGraphicsLineItem* lineitem3 = new QGraphicsLineItem;
	lineitem3->setLine(-30, 230, 30, 230);//添加栈的下边
	lineitem3->setPen(penA);
	addItem(lineitem3);
...
{
	QPointF modify(0, 200 - 50 * location);
	QGraphicsItem *hoverItem = getItemAt(getSnapped(modify));
	CItem *item = dynamic_cast<CItem*>(hoverItem);
	delete item;

...
void CMainWindow::on_actionOpen_triggered()
{
	...
	QString fileName = QFileDialog::getOpenFileName(NULL,
		title, loadName, filter, &m_lastOpenFilter);
	if (fileName.isEmpty())
		return;
...
bool CMainWindow::saveOnExit()
{
	...
	QMessageBox msgBox;
	msgBox.setWindowTitle(m_currentFileName);
	msgBox.setText(QString::fromLocal8Bit("文档已被修改。\n您要保存更改吗？"));
	msgBox.setIcon(QMessageBox::Warning);

	// 创建自定义按钮
	QPushButton* saveButton = new QPushButton(QString::fromLocal8Bit("保存"));
	QPushButton* discardButton = new QPushButton(QString::fromLocal8Bit("放弃"));
	QPushButton* cancelButton = new QPushButton(QString::fromLocal8Bit("取消"));

	msgBox.addButton(saveButton, QMessageBox::AcceptRole);
	msgBox.addButton(discardButton, QMessageBox::RejectRole);

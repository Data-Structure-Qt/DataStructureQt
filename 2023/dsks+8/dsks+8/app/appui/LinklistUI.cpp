...
void LinklistUI::animationDraw(int totaltime, int frametime)
{
	...
	{
		if (animation[totaltime][frametime].x == sss.at(i)) {
			m_scene->insertnode(animation[totaltime][frametime].x,
				animation[totaltime][frametime].data, -300, 0, true); //链表只需要便利每个结点一次即可 
			return;
		}
	}

	m_scene->insertnode(animation[totaltime][frametime].x,
		animation[totaltime][frametime].data, -300, 0, false); //链表只需要便利每个结点一次即可  
}
//↑
...
void LinklistUI::on_headinsert_btn_clicked()      //头部插入槽函数
{
	int temp = 0,j;
...
void LinklistUI::on_search_btn_clicked() //槽函数,节点查找
{
...
//↓
void LinklistUI::on_sum_btn_clicked() //并集按钮槽函数 
{
	for (int i = 1; i <= QLinkList.size(); ++i) //删除所有节点 
	{
		m_scene->deletenode(i);
	}
	QString data = ui->dataEdit1->text();
	data.remove(QRegExp("\\s* +$"));
	QStringList temp = data.split(" ");
	QString data1 = ui->dataEdit2->text();
	data1.remove(QRegExp("\\s* +$"));
	QStringList temp1 = data1.split(" ");
	for (int i = 0; i < temp.size(); i++)
	{
		m_scene->insertnode(i, temp.at(i).toInt(), -300, -300,false);//向屏幕绘制
	}
	for (int i = 0; i < temp1.size(); i++)
	{
		m_scene->insertnode(i, temp1.at(i).toInt(), -300, 0, false);//向屏幕绘制
	}
	int z = 0;
	int checked = false;
	sss.clear();
	...
	{ 
		...
		for (;j < temp.size(); j++)
		{
			if (temp1.at(i).toInt() == temp.at(j).toInt()) {
				sss.push_back(i++);
			}
			else
			{
	...
	if (checked)
	{
		...
		{
			LinklistNode* node = new LinklistNode();
			node->m_StrName = temp1.at(i);
			//添加进链表 
			QLinkList.append(*node);
			if (sss.size() > 0 && j < sss.size())
			{
				if (sss.at(j) == i) {
					m_scene->insertnode(i, temp1.at(i).toInt(), -300, 300, true);//向屏幕绘制
					j++;
				}
				else
				{
					m_scene->insertnode(i, temp1.at(i).toInt(), -300, 300, false);//向屏幕绘制
				}

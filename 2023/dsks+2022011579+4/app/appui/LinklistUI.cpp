...
void LinklistUI::on_Remove_duplicates_btn_clicked()
{
	QSet<int> visitedValues;
	...
	// 重新创建节点
	onlytotaltime = 0;
	frametime[onlytotaltime] = -1;  // 初始化frametime数组

	int i = 0;
	while (i < QLinkList.size())
	{
		LinklistNode currentNode = QLinkList.at(i);

		// 如果当前节点的值已经在集合中出现过，说明是重复元素，删除该节点
		if (visitedValues.contains(currentNode.m_StrName.toInt()))
		{
			QLinkList.removeAt(i);
		}
		else
		{
			visitedValues.insert(currentNode.m_StrName.toInt());

			// 创建节点并绘制
			m_scene->insertnode(i, currentNode.m_StrName.toInt());
			animationRecord(0, i, i, currentNode.m_StrName.toInt());
			frametime[onlytotaltime] = i;
			i++;
		}
	}

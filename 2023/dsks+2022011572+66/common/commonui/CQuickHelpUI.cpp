...
void CQuickHelpUI::setSortCode()
{
	ui->Viewer->setText("int left = 0, right = arr.size() - 1, flag;\nwhile (left < right)\n{\n\tflag = 0;\n\tif (!flag)\n\t{\n\t\tbreak;\n\t}\n\tright--;\n\t}");
}

void CQuickHelpUI::showFSort()
{
	ui->Viewer->setText("for (int i = left; i < right; i++)\n\t{\n\t\tif (arr[i] > arr[i + 1])\n\t\t{\n\t\t\tint temp = arr[i + 1];\n\t\t\tarr[i + 1] = arr[i];\n\t\t\tarr[i] = temp;\n\t\t\tflag = 1;\n\t\t}\n\t}\n\t");
}

void CQuickHelpUI::showBSort()
{
	ui->Viewer->setText("for (int i = right; i > left; i--)\n\t{\n\t\tif (arr[i] < arr[i - 1])\n\t\t{\n\t\t\tint temp = arr[i - 1];\n\t\t\tarr[i - 1] = arr[i];\n\t\t\tarr[i] = temp;\n\t\t}\n\t}\n\tleft++;\n");
}

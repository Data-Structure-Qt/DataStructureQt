...
// Forward declaration or include of MGraph
...
void GraphUI::on_reset_btn_clicked()
{
	...
	for (auto node : nodes)
	{
		...
		node->setSize(70, 70);
		node->setLabelText(node->getId());
		node->setLabelPosition(node->x() / 4096 - 15, node->y() / 4096 - 38);
		node->showLabel(true);
		//显示标签 
	}
	for (auto node : nodes)
...
void GraphUI::createGraph(MGraph &g) {
	...
	{
		if (node->getId().toStdString() !=selectedNodes.at(0)->getId().toStdString()) {
...
void GraphUI::on_articul_btn_clicked()
{
	MGraph g; //新建图
	createGraph(g); //依据屏幕边与节点创建图
	findArticul(g);
...
//获取图的关节点（割点）
int visit[2000], low[2000], c;
int degree[2000]; // 新增数组来存储每个节点的度

//void GraphUI::DFSArticul(MGraph G, int v, int parent)
//{
//	visit[v] = c++;
//	low[v] = visit[v];
//	int children = 0; // 记录子节点的数量
//
//	for (CGraphVertexNode *q = G.adjointList[v].firstEdage; q != NULL; q = q->next)
//	{
//		int w = q->adjointVertex;
//
//		if (visit[w] == 0)
//		{
//			children++;
...
//
//			if (low[w] >= visit[v] && parent != -1)
//			{
//				NodeChangeColor(QString::fromStdString(G.adjointList[v].data));
//			}
//		}
//		else if (w != parent)
//		{
...
//
//
//void GraphUI::findArticul(MGraph G)
//{
//	for (int i = 0; i < G.numVertex; i++)
//	{
//		visit[i] = 0;
...
void GraphUI::DFSArticul(MGraph G, int v, int parent)
{
	qDebug() << "Visiting node:" << v << " with parent:" << parent;
	...
	for (CGraphVertexNode *q = G.adjointList[v].firstEdage; q != NULL; q = q->next)
	{
		...
		qDebug() << "Processing edge:" << v << " -> " << w;

		if (visit[w] == 0)
		{
			children++;
			qDebug() << "Recursing into child:" << w;
			DFSArticul(G, w, v);
			qDebug() << "Returned from child:" << w;

			low[v] = min(low[v], low[w]);

			if (low[w] > visit[v] && parent != -1)
			{
				qDebug() << "Found articulation point at:" << v;
				NodeChangeColor(QString::fromStdString(G.adjointList[v].data));
			}
		}
		else if (w != parent)
	...
	{
		qDebug() << "Found articulation point at root:" << v;
		NodeChangeColor(QString::fromStdString(G.adjointList[v].data));
	}
}
...
void GraphUI::findArticul(MGraph G)
{
	...
	for (int i = 0; i < G.numVertex; i++)
	{
		if (visit[i] == 0)
		{
			qDebug() << "Starting DFS from node:" << i;
			DFSArticul(G, i, -1);
		}
	}
}

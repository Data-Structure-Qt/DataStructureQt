...
// 在LinklistUI.cpp 文件中实现以下函数

void LinklistUI::bubbleSort()
{
	int n = QLinkList.size();
	for (int i = 0; i < n - 1; ++i) {
		for (int j = 0; j < n - i - 1; ++j) {
			if (QLinkList[j].m_StrName.toInt() < QLinkList[j + 1].m_StrName.toInt()) {
				// 交换元素
				std::swap(QLinkList[j], QLinkList[j + 1]);

				// 用动画展示交换过程
				animateBubbleSortStep(j);
			}
		}
	}
}

void LinklistUI::animateBubbleSortStep(int step)
{
	// 清空场景
	for (int i = 1; i <= QLinkList.size(); ++i) {
		m_scene->deletenode(i);
	...
	// 重新绘制元素
	for (int i = 0; i < QLinkList.size(); ++i) {
		int x = QLinkList.at(i).m_StrName.toInt();
		m_scene->insertnode(i, x);
	}
	...
	for (int i = 0; i < QLinkList.size(); ++i) {
		frametime[i] = i;
		animationRecord(onlytotaltime, frametime[i], i, QLinkList[i].m_StrName.toInt());
	}

	// 等待一段时间，以展示动画效果
	sleepTime(1000);
}
void LinklistUI::on_sort_btn_clicked()
{
	createflag = 0;
	deleteflag = 0;
	searchflag = 0;

...
void BinaryTreeUI::on_binarytree_create_btn_clicked()
{
	...
	int n = 0;
	while (Haff.size() > 1) {
		sort(Haff.begin(), Haff.end(), [](const CBiTreeNode* a, const CBiTreeNode* b) {
			return a->key > b->key;//每次选出权值中未被选择过的两个最小权值
		});
		CBiTreeNode* left = Haff.back();
		Haff.pop_back();
		CBiTreeNode* right = Haff.back();
		Haff.pop_back();

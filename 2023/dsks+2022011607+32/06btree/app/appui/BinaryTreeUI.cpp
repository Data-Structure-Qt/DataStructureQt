...
void BinaryTreeUI::on_binarytree_SimulationTree_creat_btn_clicked()
{
	...
	vector<CBiTreeNode*>nodes(num);
	for (int i = 0; i < num; i++)
	{
		nodes[i]->key = list_array[i];
	}
	while (nodes.size() > 1) {
		// 排序权值最小的两个节点
		sort(nodes.begin(), nodes.end(), [](const CBiTreeNode* a, const CBiTreeNode* b) {
			return a->key > b->key;
		});

		// 取出两个权值最小的节点
		CBiTreeNode* left = nodes.back();
		nodes.pop_back();
		CBiTreeNode* right = nodes.back();
		nodes.pop_back();
...
int BinaryTreeUI::DepthOfBiTree(CBiTreeNode* root, int &In)
{
	...
	{
		m = DepthOfBiTree(root->left, In); //递归计算左子树的深度记为m
		n = DepthOfBiTree(root->right, In); //递归计算右子树的深度记为n
		if (m > n)
...
void BinaryTreeUI::on_binarytree_create_btn_clicked()
{
	...
	for (int i = 0; i < list.size(); ++i) {
		...
		if (!flag)
			return;
		list_array[i] = x;//存为数组 
		CBiTreeNode *l = new CBiTreeNode(list_array[i], NULL, NULL);
		l->height = 0;
		nodes[i] = l;
	}
	int n = 0;
	while (nodes.size() > 1) {
		...
		// 创建新节点，权值为左右子节点的权值之和
		CBiTreeNode *newNode = new CBiTreeNode(0, NULL, NULL);
		newNode->left = left;
		newNode->right = right;
		newNode->height = 0;
	...
	for (int i = 0; i < list.size() + n; i++)
	{
		//求二叉树的深度 
		int depth = 0;
		depth = DepthOfBiTree(root, depth);
		//求树的每层标准高度 

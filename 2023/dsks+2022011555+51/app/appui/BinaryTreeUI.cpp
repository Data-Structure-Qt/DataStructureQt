...
void BinaryTreeUI::on_binarytree_public_parentnode_search_btn_clicked()
{
	...
	QString data1 = list.at(0);
	QString data2 = list.at(1);
	CBiTreeNode* node1 = avlTree->search(root, data1.toInt());
	CBiTreeNode* node2 = avlTree->search(root, data2.toInt());
	...
	if (node1 == node2) {
		...
		bool find = false;
		CBiTreeNode* node1_parent = node1->parent;
		CBiTreeNode* node2_parent = node2->parent;
		...
		if (!find) {
			node1_parent = node1->parent;
			node2_parent = node2->parent;
			while (!find && node1_parent && node2_parent) {				
				while (node2_parent){

...
void CMainWindow::onQuit() { removeInstance(); }
...
QDockWidget *CMainWindow::createDockWindow(const QString &name,
                                           const QString &title,
...
void CMainWindow::dragLeaveEvent(QDragLeaveEvent *event) { event->accept(); }
...
void CMainWindow::createMainMenu() {
  ...
  // QString::fromLocal8Bit("链表代码区域")
  m_fileMenu = menuBar()->addMenu(QString::fromLocal8Bit("文件"));

  m_newDocument = m_fileMenu->addAction(QIcon(":/Icons/New"),
                                        QString::fromLocal8Bit("新建"));
  m_newDocument->setStatusTip(QString::fromLocal8Bit("创建新文件"));

  fillNewFileMenu();

  m_openDocument = m_fileMenu->addAction(QIcon(":/Icons/Open"),
                                         QString::fromLocal8Bit("打开..."),
                                         this, SLOT(on_actionOpen_triggered()));
  m_openDocument->setStatusTip(QString::fromLocal8Bit("打开一个文件"));
  m_openDocument->setShortcut(QKeySequence::Open);

  m_saveDocument = m_fileMenu->addAction(QIcon(":/Icons/Save"),
                                         QString::fromLocal8Bit("保存"), this,
                                         SLOT(on_actionSave_triggered()));
  m_saveDocument->setStatusTip(QString::fromLocal8Bit("保存当前文件"));
  m_saveDocument->setShortcut(QKeySequence::Save);

  m_saveAsDocument = m_fileMenu->addAction(tr("Save &As..."), this,
                                           SLOT(on_actionSaveAs_triggered()));
  m_saveAsDocument->setStatusTip(QString::fromLocal8Bit("将当前文件另命名为"));
  m_saveAsDocument->setShortcut(QKeySequence::SaveAs);

  m_exportDocument =
      m_fileMenu->addAction(QIcon(":/Icons/Export"), tr("&Export..."));
  m_exportDocument->setStatusTip(
      tr("Export current document into another format"));
  m_exportDocument->setVisible(false);

  m_fileMenu->addSeparator();

  m_recentFilesMenu = m_fileMenu->addMenu(QString::fromLocal8Bit("最近的文件"));
  connect(m_recentFilesMenu, SIGNAL(aboutToShow()), this,
          SLOT(fillRecentFilesMenu()));
  connect(m_recentFilesMenu, SIGNAL(triggered(QAction *)), this,
          SLOT(onRecentFilesMenuAction(QAction *)));

  m_fileMenu->addSeparator();

  QAction *closeDocument =
      m_fileMenu->addAction(tr("Close"), this, SLOT(close()));
...
void CMainWindow::fillNewFileMenu() {
  ...
  if (m_docTypeCreate.count() == 1) {
    ...
    m_newDocument->setText(QString::fromLocal8Bit("新建"));
    ...
    connect(m_newDocument, SIGNAL(triggered()), this,
    ...
    connect(m_newDocument, SIGNAL(triggered()), newActionsMenu->defaultAction(),
            SIGNAL(triggered()));

    connect(newActionsMenu, SIGNAL(triggered(QAction *)), this,
            SLOT(createNewDocument(QAction *)));
    m_newDocument->setEnabled(true);
  }
}
...
void CMainWindow::updateTitle() {
  setWindowTitle(QString("%1 - %2").arg(
      m_mainTitleText, QApplication::applicationDisplayName()));
}
...
void CMainWindow::onCurrentFileChanged() {
  ...
  if (m_currentFileName.isEmpty())
    m_mainTitleText = QString::fromLocal8Bit("");
...
void CMainWindow::selectAndOpenDocument() { on_actionOpen_triggered(); }
...
void CMainWindow::on_actionOpen_triggered() {
  QString title = QString::fromLocal8Bit("打开文件");
  ...
  QString fileName = QFileDialog::getOpenFileName(NULL, title, loadName, filter,
                                                  &m_lastOpenFilter);
  if (fileName.isEmpty())
    return;
...
bool CMainWindow::getDocFormatFromName(const QString &normalizedName,
                                       const CDocument **doc,
                                       const CDocumentFormat **fmt,
                                       QString *suffix) {
...
bool CMainWindow::doOpenDocument(const QString &fileName) {
  ...
  if (!QFile::exists(normalizedName)) {
    ...
    QMessageBox::critical(
        NULL, fileName, tr("Document file does not exist or not accessible."));
...
void CMainWindow::on_actionSave_triggered() { save(); }

void CMainWindow::on_actionSaveAs_triggered() { saveAs(); }
...
bool CMainWindow::saveAs() {
  ...
  QString fileName = QFileDialog::getSaveFileName(NULL, title, saveName, filter,
                                                  &selectedFilter);
  if (fileName.isEmpty())
    return false;
...
bool CMainWindow::doSaveDocument(const QString &fileName,
                                 const QString &selectedFilter,
                                 const QByteArray &docType) {
  if (saveDocument(fileName, selectedFilter, docType)) {
    ...
    QMessageBox::critical(
        NULL, tr("Save Error"),
        tr("Document cannot be saved. Check access rights and path."));
  }
...
bool CMainWindow::activateInstance(const QString &fileName) {
  QString normalizedName =
      fileName; // QDir::toNativeSeparators(QFileInfo(fileName).canonicalFilePath());
#ifdef WIN32
...
void CMainWindow::createWindowsMenu() {
  m_windowsMenu = new QMenu(QString::fromLocal8Bit("窗口"));
  m_windowsMenuAction = menuBar()->addMenu(m_windowsMenu);

  connect(m_windowsMenu, SIGNAL(aboutToShow()), this, SLOT(fillWindowsMenu()));
  connect(m_windowsMenu, SIGNAL(triggered(QAction *)), this,
          SLOT(onWindowsMenuAction(QAction *)));
}
...
void CMainWindow::createHelpMenu() {
  m_helpMenu = new QMenu(QString::fromLocal8Bit("帮助"));

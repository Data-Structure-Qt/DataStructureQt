...
void BinaryTreeUI::MaxLeaf(CBiTreeNode* root, int l, int max)
{
	if (root == NULL) return;
	else
	{
		...
		MaxLeaf(root->left, l + 1, max);
		MaxLeaf(root->right, l + 1, max);
	}
}
void BinaryTreeUI::Maxa(CBiTreeNode* root, int l, int max, int a[]) {
	if (root == NULL) return;
	else
	{
		if (root->left == NULL && root->right == NULL)
		...
		Maxa(root->left, l + 1, max, a);
		Maxa(root->right, l + 1, max, a);
	}
}
void BinaryTreeUI::on_binarytree_node_far_btn_clicked() {
	int l = 0, max = 0;
	int a[20];
	int num = 0;
	MaxLeaf(root, l, max);

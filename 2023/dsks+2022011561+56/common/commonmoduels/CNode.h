...
public:
	...
	CNode(QGraphicsItem* parent = NULL,int falg = 0);
	virtual ~CNode();
	...
	// reimp
	virtual CItem* create() const		{ return new CNode(parentItem(),0); }
	virtual CItem* searchtag() const	{ return new CNode(parentItem(),1); }
	virtual CItem* clone();

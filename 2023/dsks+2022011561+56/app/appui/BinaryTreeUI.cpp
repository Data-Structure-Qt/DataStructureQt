	...
	m_scene(NULL)
{
	...
	listshow = nullptr;
	// qt自动连接
	//connect(ui->btnInsert, &QPushButton::clicked, this, &BinaryTreeUI::on_btnInsert_clicked);
	//connect(ui->btnDelete, &QPushButton::clicked, this, &BinaryTreeUI::on_btnDelete_clicked);
	paintWidget = new BTreeWidget();
	ui->BinaryTreeDataEdit->hide();
	ui->Button_CleanBiTreeNode->hide();
	ui->Button_DeleteBiTreeNode->hide();
	ui->Button_InsertBiTreeNode->hide();
	ui->Button_SearchBiTreeNode->hide();
	ui->label->hide();
}

void BinaryTreeUI::on_btnInsert_clicked()
{
	int key = ui->BinaryTreeDataEdit_2->text().toInt();
	paintWidget->insert(key);
	// 获取paintWidget的宽度和高度
	int widgetWidth = paintWidget->width();
	int widgetHeight = paintWidget->height();

	m_scene->setSceneRect(0, 0, widgetWidth, widgetHeight);
	int sceneWidth = m_scene->width();
	int sceneHeight = m_scene->height();
	int centerX = (sceneWidth - widgetWidth) / 2;
	int centerY = (sceneHeight - widgetHeight) / 2;
	paintWidget->move(centerX, centerY);

	m_quickHelp->showMsg(QString::fromLocal8Bit("寻找位置，进行插入%1。\n插入成功").arg(QString::number(key)));

}

void BinaryTreeUI::on_btnDelete_clicked()
{
	int key = ui->BinaryTreeDataEdit_2->text().toInt();
	paintWidget->deleteKey(key);
	m_quickHelp->showMsg(QString::fromLocal8Bit("寻找位置，进行删除%1。\n删除成功").arg(QString::number(key)));

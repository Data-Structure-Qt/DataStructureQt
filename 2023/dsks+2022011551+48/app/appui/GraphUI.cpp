...
{
	ui->setupUi(this);
	connect(ui->aoeButton, &QPushButton::clicked, this, &GraphUI::onAOEButtonClicked);
}
...
#include "AOEUtil.h"
void GraphUI::onAOEButtonClicked()
{
	QList<CNode*> nodes = m_scene->getItems<CNode>();
	QList<CEdge*> edges = m_scene->getItems<CEdge>();
	for (auto edge : edges)//显示边标签
	{
		QRegularExpression pattern("\\d+");  // 匹配一个或多个数字
		QRegularExpressionMatchIterator matches = pattern.globalMatch(edge->getId());
		QString result;
	...
	for (auto edge : edges)
	{
		auto data1 = edge->firstNode()->getId();
		auto data2 = edge->lastNode()->getId();
		...
		auto weight = result.toInt();
		graph[data1][data2] = weight;
	}

	auto myresult = findCriticalActivities(graph);
	for (auto edge : edges)
	{
		auto data1 = edge->firstNode()->getId();
		auto data2 = edge->lastNode()->getId();
		if (myresult.contains(data1) && myresult[data1].contains(data2) && myresult[data1][data2] == true)
		{
			edge->setAttribute("color", "#c55159");
		}
...
void GraphUI::DFSTraverse(MGraph g) {//节点初始化 未访问
	for (int i = 0; i < g.numVertex; i++) {
		Mvisited[i] = false;
	}
	//对未访问的节点进行遍历
...
{
	QTime dieTime = QTime::currentTime().addMSecs(time);//获取自定义休眠时	间

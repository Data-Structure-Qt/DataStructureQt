...
void CNodeEditorScene::setEdgesFactory(CEdge* edgeFactory)//创建初始数据，即关键字序列
{
...
void CNodeEditorScene::drawText(int location)
{
	...
	pItem->setPlainText(QString::fromLocal8Bit("关键字序列"));  // 纯文本
	...
	//pItem1->setPlainText(QString::fromLocal8Bit("关键字序列"));  // 纯文本
	...
	//pen.setColor(QColor(100, 149, 237));//外边框的颜色 蓝色
	//pen.setWidth(5);
	//lineItem->setPen(pen);
	...
	lineItem1->setLine(-175 - 50, -300, -175 + 50 * (location)-25 - 25 + 100,-300);//(a,b,c,d) a->x b=d为直线且->y  c->线段终点
	//location=7   
	//原始数据(-300 - 50, -20, -300 + 50 * (location)-25 - 25 + 100, -20)
	addItem(lineItem1);
}
void CNodeEditorScene::drawResult(int location)//打印“桶排序结果”
{
	// 正方形  长度25一格 正负40格 
	//绘制文字
	QGraphicsTextItem *pItem = new QGraphicsTextItem();
	pItem->setPlainText(QString::fromLocal8Bit("桶排序结果"));  // 纯文本
	pItem->setDefaultTextColor(QColor(255,0,0));  // 文本色
	pItem->setPos(QPointF(-125, 200));
	//位置 x ,y<0的情况下： |y|越大越靠上，|x|越大越靠左 x=-125 居中 
	//x<0靠左 y<0靠右
	QFont font = pItem->font();
	font.setPixelSize(50);  // 像素大小 每一格大小25左右
	...
	pen1.setWidth(5);
	lineItem1->setPen(pen1);
	lineItem1->setLine(-175 - 50, 350, -175 + 50 * (location)-25 - 25 + 100, 350);//(a,b,c,d) a->x b=d为直线且->y  c->线段终点
	//location=7   
	//原始数据(-300 - 50, -20, -300 + 50 * (location)-25 - 25 + 100, -20)
	addItem(lineItem1);
}
void CNodeEditorScene::drawTong(int location)//打印“桶排序过程”
{
	// 正方形  长度25一格 正负40格 
	//绘制文字
	QGraphicsTextItem *pItem = new QGraphicsTextItem();
	pItem->setPlainText(QString::fromLocal8Bit("桶排序过程"));  // 纯文本
	pItem->setDefaultTextColor(QColor(0, 0, 255));  // 文本色
	pItem->setPos(QPointF(-500, -125));
	//位置 x ,y<0的情况下： |y|越大越靠上，|x|越大越靠左 x=-125 居中 
	//x<0靠左 y<0靠右
	QFont font = pItem->font();
	font.setPixelSize(35);  // 像素大小 每一格大小25左右
	pItem->setFont(font);
	addItem(pItem);
...
void CNodeEditorScene::tongResult(int location, int data)//屏幕显示桶排序结果
{
	if (location == 0)
	{
		orgin.setX(-175);//第一个矩形中心的位置
...
void CNodeEditorScene::tong(int i, int location, int data)//桶排序屏幕实现 i表示第几桶
{
	...
	auto node1 = createNewNode(getSnapped(o));//绘制矩形
	node1->setSelected(false);
	QGraphicsItem *hoverItem = getItemAt(getSnapped(o));
	CItem *item = dynamic_cast<CItem*>(hoverItem);
	onActionEditLabel(item, data);
	if (flag)
	{

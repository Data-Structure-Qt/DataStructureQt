...
void BinaryTreeUI::on_recoverTree_btn_clicked() {
    QString inorderSequence = ui->inorderSeqEdit->text();
    QString postorderSequence = ui->postorderSeqEdit->text();

    if (inorderSequence.size() != postorderSequence.size()) {
        QMessageBox::warning(
    ...
    // 构建中序和后序遍历共同构成的二叉树
    root = avlTree->buildTreeFromInorderPostorder(inorderSequence, postorderSequence);
    int depth = 0;

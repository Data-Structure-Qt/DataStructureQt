...
//求结点高度函数
int LevelOfBiTreeNode(CBiTreeNode* root, int key, int level)
{
	if (root == nullptr) {
		return 0;
	}
	...
	int left_level = LevelOfBiTreeNode(root->left, key, level + 1);
	if (left_level != 0) {
		return left_level;
	}

	int right_level = LevelOfBiTreeNode(root->right, key, level + 1);
	return right_level;
}
//平衡二叉树节点查找函数
void BinaryTreeUI::on_binarytree_node_search_btn_clicked1()
{
	QString data = ui->BinaryTreeDataEdit_2->text();
	bool flag;
	int x = data.toUInt(&flag);
...
void BinaryTreeUI::on_binarytree_node_search_btn_clicked()
{
	...
	history_highSize[0] = highSize;
	int x = 0;
	int minhigh = 100;//最小深度
	int maxcount = 0;//创建输入端最大数字
	CBiTreeNode* node = avlTree->search(root, 0);
while(x<10)
{
	CBiTreeNode* node = avlTree->search(root, x);
	if (node != NULL)
	 node->height = LevelOfBiTreeNode(root, node->key, 0);
	if (node != NULL && node->left == NULL && node->right == NULL) 
	{
			x = node->key;
			if (node->height < minhigh)
			{
				minhigh = node->height;
			}
	}
	...
	while (x < 10) {
		...
		//适应网格
		m_scene->addUndoState();
		node = avlTree->search(root, x);
		for (int j = 0; j <= framesearchtime; j++)
		{
			if (node != NULL && node->left == NULL && node->right == NULL && node->height == minhigh) {
				x = node->key;
				animationDrawSearch(0, j, x);
			}
			sleepTime(75);

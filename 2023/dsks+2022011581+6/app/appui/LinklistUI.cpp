...
void LinklistUI::on_parity_btn_clicked()
{

	CNode* porigin_node = this->m_scene->getNodesFactory();
	if (porigin_node == nullptr)
	{
		return;
	}
	...
	for (int i = 0; i < QLinkList.size(); ++i)
	{
		if (i % 2 == 0)
		{
			this->evenList.push_back(QLinkList.at(i));
		}
		else
		{
			this->oddList.push_back(QLinkList.at(i));
		}
	}
	
	QPointF origin_pos = porigin_node->pos();
	const int y_setoff = 100;
	const int x_setoff = 150;
	QPointF odd_pos(origin_pos.x(), origin_pos.y() + y_setoff);
	QPointF even_pos(origin_pos.x(), odd_pos.y() + y_setoff);
	
	CNode* podd_node = nullptr;
	CNode* podd_next_node = nullptr;
	for (auto &_it  : this->evenList)
	{
		QString tmp = _it.m_StrName;
		bool flag;
		int x = tmp.toUInt(&flag);
		if (!flag)
			return;
		...
		podd_node = podd_next_node;
		podd_next_node = m_scene->createNewNode((odd_pos));//向屏幕绘制数据节点
		if (podd_next_node != nullptr && podd_node != podd_next_node)
		{
			m_scene->createNewConnection(podd_node, podd_next_node);
		}
		podd_next_node->setSelected(false);
		podd_next_node->setLabelText(tmp);
		odd_pos = odd_pos + QPointF(50, 0);
		podd_next_node = m_scene->createNewNode((odd_pos));	
		
		podd_next_node->setSelected(false);
		
		odd_pos = odd_pos + QPointF(x_setoff, 0);
	}

	CNode* peven_node = nullptr;
	CNode* peven_next_node = nullptr;
	for (auto& _it : this->oddList)
	{
		QString tmp = _it.m_StrName;
		bool flag;
		int x = tmp.toUInt(&flag);
		...
		peven_node = peven_next_node;
		peven_next_node = m_scene->createNewNode(m_scene->getSnapped(even_pos));
		if (peven_next_node != nullptr && peven_node != peven_next_node)
		{
			m_scene->createNewConnection(peven_node, peven_next_node);
		}
		peven_next_node->setSelected(false);
		peven_next_node->setLabelText(tmp);
		even_pos = even_pos + QPointF(50, 0);
		peven_next_node = m_scene->createNewNode(m_scene->getSnapped(even_pos));

		even_pos = even_pos + QPointF(x_setoff, 0);
	}
}

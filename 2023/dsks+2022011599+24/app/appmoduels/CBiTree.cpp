...

{
	...
	if (node->right == NULL)
		...
		return node; return maximus(node);
...

{
	...
	if (root) {
		...
		In += inStr; InOrder(root->right, In);
...

{
	...
	linenum = linenum - 1; while (!q.empty()) {
...

		{
			...
			//遍历同一行的结点 按照它们在中序序列的位置 修改在 line  中位置
...

				{
					...
					line[InOrd.find(p->key)] = char(p->key); for (int j = 0; j < InOrd.find(inStr); j++)

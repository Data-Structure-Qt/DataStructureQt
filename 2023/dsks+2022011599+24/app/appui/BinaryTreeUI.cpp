...
// 递归函数
void inOrderTraversal(CBiTreeNode* root, int& count, int k, int& result) {
	if (root == nullptr) {
		return;
	}
...
// 找出第 k 个节点的值
void findKthNodeAndMarkRed(CBiTreeNode* root, int k,int& r) {
	int count = 0;
	 
	inOrderTraversal(root, count, k, r);//调用递归函数，进行中序遍历
	 // 用 r 记录第 k 个点的值
}
...
void BinaryTreeUI::on_binarytree_node_find_btn_clicked()
{
	...
	int r;
	findKthNodeAndMarkRed(root, x, r);
		inOrderPrint_search(root, 0, 0, wideSize, highSize, 0, 0,r , 0);
		m_scene->clearScreen();//清屏 
		for (int j = 0; j <= frametime[onlytotaltime]; j++)
		{
			animationDraw(onlytotaltime, j);

...
void GraphUI::createGraph(MGraph &g) {
	...
	for (auto node : nodes)// 其余进入数组
	{
		if (node->getId().toStdString() !=
			...
			g.adjointList[nodei].data = node->getId().toStdString();// 转为 string类型
				g.adjointList[nodei].firstEdage = NULL;
			nodei++;
		}
	}
...
void GraphUI::BFSTraverse(MGraph g) {
	...
	for (int i = 0; i < g.numVertex; i++) {
		if (!Mvisited[i]) {
			...
			// 节点改变颜色
			NodeChangeColor_2(QString::fromStdString(g.adjointList[i].data));
			animationRecord(1,
			...
			while (!bfsQueue.empty()) {
				...
				while (node) {
					if (!Mvisited[node->adjointVertex]) {
						...
						EdgeChangeColor_2(QString::fromStdString(g.adjointList[queueValue].data),
						...
						animationRecord(2, QString::fromStdString(g.adjointList[queueValue].data),
							QString::fromStdString(g.adjointList[node->adjointVertex].data));
						NodeChangeColor_2(QString::fromStdString(g.adjointList[node->adjointVertex
						].data));
						animationRecord(1,
...
// 改变节点的颜色
void GraphUI::NodeChangeColor_1(QString nodeID)
{
	QList<CNode*> nodes = m_scene->getItems<CNode>();
...
void GraphUI::NodeChangeColor_2(QString nodeID)
{
	QList<CNode*> nodes = m_scene->getItems<CNode>();
	for (auto m_node : nodes) {
		if (m_node->getId() == nodeID) {
			for (int i = 0; i < 2; i++)// 闪烁
			{
				...
				while (time.elapsed() < 300); // 等待时间
				QCoreApplication::processEvents(); // 处理事件
				m_node->setAttribute("color", "#0000ff");
				time.start();
				while (time.elapsed() < 300); // 等待时间
				QCoreApplication::processEvents(); // 处理事件
			}
...
// 改变线段的颜色
void GraphUI::EdgeChangeColor_1(QString firstNodeID, QString
	lastNodeID)
{
	QList<CEdge*> edges = m_scene->getItems<CEdge>();
...
void GraphUI::EdgeChangeColor_2(QString firstNodeID, QString
	lastNodeID)
{
	QList<CEdge*> edges = m_scene->getItems<CEdge>();
	for (auto m_edge : edges)
	{// 前后节点连接的边
			...
			QString(m_edge->lastNode()->getId()) == lastNodeID)
		{
			for (int i = 0; i < 2; i++)// 闪烁
			{
				...
				while (time.elapsed() < 300); // 等待时间
				QCoreApplication::processEvents(); // 处理事件
				m_edge->setAttribute("color", "#0000ff");
				time.start();
				while (time.elapsed() < 300); // 等待时间
				QCoreApplication::processEvents(); // 处理事件
			}
...
void GraphUI::DFS(MGraph g, int index) {
	...
	NodeChangeColor_1(QString::fromStdString(g.adjointList[index].data));
	...
	while (node) {
		...
		if (!Mvisited[node->adjointVertex]) {
			EdgeChangeColor_1(QString::fromStdString(g.adjointList[index].data),
				QString::fromStdString(g.adjointList[node->adjointVertex].data));
...
void GraphUI::MiniSpanTree_PRIM(AMLGraph G) {
	...
	int wei = 0; for (int w = FirstAdjVex(G, i0, wei); w >= 0; w = NextAdjVex(G, i0, w, wei)) {
		close[w].lowcost = wei;
	}
	close[i0].lowcost = 0;
	string predata = v0;// 前一个节点
	NodeChangeColor_1(QString::fromStdString(predata));
	...
	for (int i = 0; i < G.vexnum - 1; i++)
	{
		...
		// 输出节点
		NodeChangeColor_1(QString::fromStdString(G.adjmulist[minadj].data));
		...
		for (auto edge : edges)
		{
			if (edge->getId() == QString::number(min) && edge->getAttribute("color") !=
				"#FF0000")
			{
				EdgeChangeColor_1(edge->firstNode()->getId(),
					edge->lastNode()->getId());
...
void GraphUI::MiniSpanTree_Kruskal(AMLGraph G) {
	...
	for (int i = 0; i < edg.size(); i++) {
		...
		if (e1 != e2) {
			...
			{
				NodeChangeColor_1(QString::fromStdString(G.adjmulist[edg[i].u].data));
			...
			{
				NodeChangeColor_1(QString::fromStdString(G.adjmulist[edg[i].v].data));
...
void GraphUI::topoSort(topoNode topoadj[], int n)
{
	...
	while (mystack.size() != 0)
	{
		...
		NodeChangeColor_1(toponodeID.data());
...
void GraphUI::animationDraw(int totaltime)
{
	if (animation[totaltime].functionid == 1) {
		NodeChangeColor_1(animation[totaltime].para1);
	}
	else if (animation[totaltime].functionid == 2) {
		EdgeChangeColor_1(animation[totaltime].para1,

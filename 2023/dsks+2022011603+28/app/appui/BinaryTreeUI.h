...
private slots:
	...
	//最近叶子结点
	void on_binarytree_node_near_btn_clicked();
	//平衡二叉树递归展示函数
	void inOrderPrint(CBiTreeNode * p, double x, double y,
		double wideSize, double highSize, double p_x, double p_y, int frame);
	//用于搜索的平衡二叉树递归展示函数
	void inOrderPrint_search(CBiTreeNode * p, double x, double y,
	...
	void MinLeafa(CBiTreeNode* b, int l, int min);
	//最近叶节点
	void Mina(CBiTreeNode* b, int l, int min, int a[]);

...
void BinaryTreeUI::MinLeafa(CBiTreeNode* p, int l, int min)
{
	if (p == NULL) return;
	else
	{
		...
		MinLeafa(p->left, l + 1, min);
		MinLeafa(p->right, l + 1, min);
	}
}
void BinaryTreeUI::Mina(CBiTreeNode* p, int l, int min, int a[]) {
	if (p == NULL) return;
	else
	{
		if (p->left == NULL && p->right == NULL)
		...
		Mina(p->left, l + 1, min, a);
		Mina(p->right, l + 1, min, a);
	}
}
void BinaryTreeUI::on_binarytree_node_near_btn_clicked() {
	int l = 0 , min = 75;
	int a[20];
	int num = 0;
	CBiTreeNode*p = root;
	...
	Mina(root, l, min, a);
		...
		//适应网格
		m_scene->addUndoState();
		animationDrawSearch(0, framesearchtime, x);
		timestamp = 0;

...
{
	m_scene->clearScreen();// Clear the screen
	maxHeap = new MaxHeap();// Create a new max heap
	...
	for (int i = 0; i < list.size(); ++i) {
		...
		if (!flag)
			return;
		list_array[i] = x; // Store elements in an array
	}
	for (int i = 0; i < list.size(); i++)
	{
		maxHeap -> insert(list_array[i]); // Insert elements into the max heap
	}
	
	// Visualize the max heap
	visualizeMaxHeap();
	// Other visualization and UI updates
	// ...
}

void BinaryTreeUI::visualizeMaxHeap()
{
	m_scene->clear();
	// Set the position for the root node
	int rootX = 0;
	int rootY = -200;
	// Create a visual representation of the root node
	QGraphicsEllipseItem* rootNode = m_scene->addEllipse(rootX, rootY, 70, 70, QPen(Qt::black), QBrush(Qt::yellow));
	QGraphicsTextItem* rootText = m_scene->addText(QString::number(list_array[0]));
	rootText->setPos(rootX + 20, rootY + 20);
	// Add the root node to the scene
	m_scene->addItem(rootNode);
	m_scene->addItem(rootText);
	// Draw edges and nodes for the rest of the heap
	int level = 1;
	int xSpacing = 200;
	int ySpacing = 200;
	int currentX = rootX;
	...
	{
		if (i == pow(2, level) - 1) // Move to the next level
		{
			level++;
			currentY += ySpacing;
			currentX = rootX - xSpacing * (pow(2, level - 1) / 2);
		}
		currentX += xSpacing;
		// Create a visual representation of the current node
		QGraphicsEllipseItem* currentNode = m_scene->addEllipse(currentX, currentY, 70, 70, QPen(Qt::black), QBrush(Qt::yellow));
		QGraphicsTextItem* currentText = m_scene->addText(QString::number(list_array[i]));
		currentText->setPos(currentX + 10, currentY + 10);
		// Add the current node to the scene
		m_scene->addItem(currentNode);
		m_scene->addItem(currentText);
		// Draw an edge from the current node to its parent
		int parentIndex = (i - 1) / 2;
		if (parentIndex >= 0)
		{
			int parentX = rootX - xSpacing * (pow(2, level - 2) / 2) + xSpacing * (i - pow(2, level - 1));
			int parentY = currentY - ySpacing;
			QGraphicsLineItem* edge = m_scene->addLine(currentX + 25, currentY, parentX + 25, parentY + 50, QPen(Qt::black));
...
void BinaryTreeUI::on_binarytree_node_delete_btn_clicked()
{
	...
	double wide = pow(2, depth) - 1;//求树的最后一层间隔宽度
...
void BinaryTreeUI::inOrderPrint(CBiTreeNode* root, double x, double y, double wideSize, double highSize, double p_x, double p_y, int frame)
{
	if (root == NULL)
		return;
	wideSize = wideSize / 2;//每次将单位值除以二，绘制在根节点的左右
		if(root->left != NULL)
		{
			...
			animationRecord(onlytotaltime, frametime[frame], x, y, -2, -1, p_x, p_y);//访问左孩子
	...
	if (root->right != NULL)
	{
		...
		//访问右孩子
		inOrderPrint(root->right, x + wideSize, y + highSize, wideSize, highSize, x, y, frame);
	}
	if (root != NULL)
	{
		frametime[frame]++;
...
void BinaryTreeUI::inOrderPrint_search(CBiTreeNode* root, double x,double y, double wideSize, double highSize, double p_x, double p_y, int data, int frame)
{
	...
	{
		inOrderPrint_search(root->left, x - wideSize, y + highSize, wideSize, highSize, x, y, data, 1);
	}
	if (root->right != NULL && data > root->key)
	{
		inOrderPrint_search(root->right, x + wideSize, y + highSize, wideSize, highSize, x, y, data, 2);
	}
}
void BinaryTreeUI::inOrderPrint_SimulationTree(CBiTreeNode* root, double x,double y, double Branch_length, double Branch_angle, double Change_angle,double p_x, double p_y)//递归展示函数
{
	if (root == NULL)
		return;
	if (root->left != NULL)
	{
		double angle = Branch_angle + Change_angle;//逆时针角度
			...
			inOrderPrint_SimulationTree(root->left, x + Branch_length_x , y - Branch_length_y, Branch_length * 0.8, angle, Change_angle, x, y);//左孩子
	}
	if (root->right != NULL)
	{
		double angle = Branch_angle - Change_angle + 2;//顺时针角度
			...
			double Branch_length_x = Branch_length * cos(radian);//x位移
			double Branch_length_y = Branch_length * sin(radian);//y位移
			inOrderPrint_SimulationTree(root->right , x + Branch_length_x,y - Branch_length_y, Branch_length * 0.8, angle, Change_angle, x, y);//右孩子
	}//绘制当前节点
		if (root != NULL)
...
void BinaryTreeUI::InOrderBiTree(CBiTreeNode* root, string &In)//中序遍历
{
	if (root) {
...
void BinaryTreeUI::animationRecord(int totaltime, int frame, int x, int y, int data1, int data2, int p_x, int p_y)
{
...
void BinaryTreeUI::animationSearchRecord(int totaltime, int frame, int x, int y, int data1, int data2, int p_x, int p_y, int flag, int codeid)
{
...
void BinaryTreeUI::animationDraw(int totaltime, int frametime)
{
	for (int i = 0; i < frametime; i++)//遍历整个动画
	{
		...
		{
			m_scene->drawCircle(animation[totaltime][frametime].x,animation[totaltime][frametime].y,animation[totaltime][frametime].data1,animation[totaltime][frametime].p_x, animation[totaltime][frametime].p_y);
		}
		else if (animation[totaltime][frametime].data1 != -1) 
		{
			m_scene->drawCircle2(animation[totaltime][frametime].x,animation[totaltime][frametime].y,animation[totaltime][frametime].data1,animation[totaltime][frametime].data2,animation[totaltime][frametime].p_x, animation[totaltime][frametime].p_y);
		}
	}
}
void BinaryTreeUI::animationDrawSearch(int totaltime, int frametime, int x)
{
	for (int i = 0; i < frametime; i++)//遍历整个动画
	{
		...
		{
			m_scene->drawCircle_search(animationSearch[totaltime][frametime].x,animationSearch[totaltime][frametime].y, animationSearch[totaltime][frametime].data1,animationSearch[totaltime][frametime].p_x,animationSearch[totaltime][frametime].p_y, x);
		}
		else if (animation[totaltime][frametime].data1 != -1)
		{
			m_scene->drawCircle_search(animationSearch[totaltime][frametime].x,animationSearch[totaltime][frametime].y, animationSearch[totaltime][frametime].data1,animationSearch[totaltime][frametime].p_x,animationSearch[totaltime][frametime].p_y, x);

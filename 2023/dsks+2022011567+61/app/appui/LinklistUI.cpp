...
void LinklistUI::on_createButton1_clicked()
{
	//先清空
	on_cleanButton1_clicked();
	...
	QList<CNode*> nodes = m_scene->getSelectedNodes();
	QString data1 = ui->dataEdit1->text();
	QString data2 = ui->dataEdit2->text();
	QString data3 = ui->dataEdit3->text();
	QString data4 = ui->dataEdit4->text();
	QString data5 = ui->dataEdit5->text();

	QStringList temp1 = data1.split(",");
	QStringList temp2 = data2.split(",");
	QStringList temp3 = data3.split(",");
	QStringList temp4 = data4.split(",");
	QStringList temp5 = data5.split(",");

	// 拼接所有的QStringList
	QStringList allData = temp1 + temp2 + temp3 + temp4 + temp5;

	// 转换为整数列表
	QList<int> intList;
	for (const QString& value : allData) 
	{
	...
	//m比链表的数多，直接返回
	int m = ui->dataEdit_m->text().toInt();
	if (m >= intList.size())
		return;
	...
	{
		LinklistNode* node = new LinklistNode();
		node->m_StrName = QString::number(intList.at(i)); // 将整数转换为字符串
		// 添加进链表
		QLinkList.append(*node);
	}
...
void LinklistUI::on_cleanButton1_clicked()
{
	m_scene->clearScreen();
	//ui->dataEdit1->clear();
	//ui->dataEdit2->clear();

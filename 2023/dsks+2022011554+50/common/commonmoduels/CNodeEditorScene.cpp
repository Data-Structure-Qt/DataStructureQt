...
void CNodeEditorScene::drawCircle_search2(int x, int y, int data, int p_x, int p_y, int data1)//绘制圆圈+箭头 颜色
{
	QGraphicsLineItem *lineItem = new QGraphicsLineItem();
	QPen pen2 = lineItem->pen();//画笔 绘制外框
	pen2.setColor(QColor(100, 149, 237));//外边框的颜色
...
//绘制圆顶点
void CNodeEditorScene::drawCircle(int x, int y, int data, QColor color/* = QColor(100, 149, 237)*/)
{
	QGraphicsEllipseItem *rectItem = new QGraphicsEllipseItem;//绘制圆形
	QPen pen = rectItem->pen();//画笔 绘制外框
	pen.setColor(Qt::green);//外边框的颜色
	rectItem->setPen(pen);
	rectItem->setBrush(QBrush(color));//填充颜色
	rectItem->setRect(QRectF(x, y, 30, 30));
	addItem(rectItem);
	...
	pItem->setPlainText(QString::number(data));  // 纯文本
	pItem->setDefaultTextColor(QColor(0, 0, 0));  // 文本色
	pItem->setPos(QPointF(x + 8, y + 5));//位置
	QFont font = pItem->font();
	font.setPixelSize(12);  // 像素大小
	pItem->setFont(font);
	addItem(pItem);
...
//绘制连线
void CNodeEditorScene::drawLine(int x1, int y1, int x2, int y2, QColor color/* = QColor(100, 149, 237)*/)
{
	QGraphicsLineItem *lineItem = new QGraphicsLineItem();
	QPen pen2 = lineItem->pen();//画笔 绘制外框
	pen2.setColor(color);//外边框的颜色
	pen2.setWidth(2);
	lineItem->setPen(pen2);

...
// 寻找层数最高的叶子节点的值
void BinaryTreeUI::findMaxHeightLeafValue(CBiTreeNode* root, int currentHeight, int& maxHeight, vector<int>& result)
{
	if (root == nullptr)
	{
		return;
	...
	// 递归遍历左子树和右子树
	findMaxHeightLeafValue(root->left, currentHeight + 1, maxHeight, result);
	findMaxHeightLeafValue(root->right, currentHeight + 1, maxHeight, result);
}
//平衡二叉树节点查找函数
void BinaryTreeUI::on_binarytree_node_search_btn_clicked()
{
	...
	int maxHeight = 0;
	vector<int> result;
	findMaxHeightLeafValue(root, 1, maxHeight, result);
	for (int j = 0; j <= frametime[onlytotaltime]; j++)
	{
		animationDraw(onlytotaltime, j);
	}

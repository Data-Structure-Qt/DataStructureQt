...
void BinaryTreeUI::on_binarytree_create_btn_clicked()
{
	...
	std::sort(list_array, list_array + list.size());
	for (int i = 0; i < list.size(); i++)
	{
		root = avlTree->insert(root, list_array[i]);//将元素插入到最大堆中
		//求二叉树的深度
		int depth = 0;
	...
	//开始显示序列
	string PreOrd;
	PreOrderBiTree(root, PreOrd);//获取最大堆的先序遍历序列
	QString temp = "DLR:" + QString::fromStdString(PreOrd);
	m_scene->printInOrd(100, -200, temp);
	string InOrd;
	InOrderBiTree(root, InOrd);//获取最大堆的中序遍历序列
	temp = "LDR:" + QString::fromStdString(InOrd);
	m_scene->printInOrd(100, -170, temp);
	string PostOrd;
	PostOrderBiTree(root, PostOrd);//获取最大堆的后序遍历序列
	temp = "LRD:" + QString::fromStdString(PostOrd);
...
//二叉树节点插入函数
void BinaryTreeUI::on_binarytree_node_insert_btn_clicked()
{
	QString data = ui->BinaryTreeDataEdit_2->text();
	bool flag;
	int x = data.toUInt(&flag);
...
//二叉树堆顶元素删除
void BinaryTreeUI::on_binarytree_node_deletetop_btn_clicked()
{
	int Maxelem=avlTree->deleteTopNode(root);
	list_array2[num] = Maxelem;
	num++;
	//开始显示序列
	string InOrd;
	PreOrderBiTree(root, InOrd);
...
void BinaryTreeUI::inOrderPrint_SimulationTree(CBiTreeNode* root, double x,
	...
	double p_x, double p_y)//递归展示最大堆函数
{
	if (root == NULL)
		return;
	if (root->left != NULL)
...
void BinaryTreeUI::on_binarytree_SimulationTree_out_btn_clicked()
{
	m_scene->clearScreen(); // 清屏
	// 获取用户输入的 K 值
	...
	for (int i = 0; i < k && i < list.size(); i++) {
		on_binarytree_node_deletetop_btn_clicked();
		sleepTime(1500);
	}
	on_binarytree_final_btn_clicked(k);
	m_quickHelp->btree_create_code(insertIdCode[frametime[onlytotaltime]]);
	// 适应窗口
	m_editorView->fitToView();
	// 适应网格
...
void BinaryTreeUI::on_binarytree_final_btn_clicked(int k)
{
	avlTree = new CBiTree();//新建最大堆对象
	root = NULL;//根节点初始化为NULL
	std::sort(list_array2, list_array2 + k, std::greater<int>());
	for (int i = 0; i < k; i++)
	{
		root = avlTree->insert(root, list_array2[i]);//将元素插入到最大堆中
	}
	num = 0;
	//开始显示序列
	string PreOrd;
	PreOrderBiTree(root, PreOrd);
	QString temp = "Final:" + QString::fromStdString(PreOrd);
	m_scene->printInOrd(-150, 200, temp);
}

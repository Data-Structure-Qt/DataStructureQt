...
//以下是交换左右子树
void BinaryTreeUI::on_binarytree_exchangecreate_btn_clicked()
{
	m_scene->clearScreen();//清屏
	avlTree = new CBiTree();//新建树
	root = avlTree->getRoot();//新建节点
	...
	CBiTreeNode *Queue[100];
	int front = -1;
	int rear = 0;
	Queue[rear] = root;
	while (rear != front)
	{
		CBiTreeNode *p = Queue[++front];
		CBiTreeNode *tem = p->left;
		p->left = p->right;
		p->right = tem;
...
void BinaryTreeUI::on_binarytree_node_delete_btn_clicked()
{
	...
	if (node == NULL)
		CBiTreeNode* dnode = avlTree->insert(root, x);
	m_scene->clearScreen();//清屏
	//开始显示序列
	string InOrd;
	PreOrderBiTree(root, InOrd);

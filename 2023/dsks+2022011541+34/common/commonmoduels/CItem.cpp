...
// bool CItem::setValueAttribute(const QByteArray& attrValue, const QVariant& v)
//{
//	setItemStateFlag(IS_Attribute_Changed);
//
//	if (attrValue == "value")
...
// bool CItem::setDefaultValue()
//{
//	if (m_value.isEmpty())
//	{
//		m_value = createNewValue();
//		return true;
//	}
//
//	return false;
...
void CItem::updateLabelContent() {
  ...
  if (!(m_internalStateFlags & IS_Attribute_Changed) &&
      !(scene->itemLabelsEnabled()) && !(scene->itemLabelsNeedUpdate()))
    return;
...
void CItem::setLabelText(const QString &text) {
  if (m_labelItem) {
    ...
    const QBrush colorBrush(QColor(242, 228, 68, 255));
...
void CItem::setLabelPosition(double posx, double posy) {
  if (m_labelItem)
    m_labelItem->setX(posx);
  m_labelItem->setY(posy);
}
...
void CItem::onItemRestored() { updateCachedItems(); }

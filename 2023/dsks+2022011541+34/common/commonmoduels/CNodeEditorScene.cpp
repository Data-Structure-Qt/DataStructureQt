...
void CNodeEditorScene::initialize() {
  ...
  // default node attr
  CAttribute nodeAttr("color", "Color", QColor("#3399ff"),
                      ATTR_FIXED); //初始为蓝色
  ...
  createClassAttribute("node", "size", "Size", QSizeF(70.0, 70.0),
  ...
  createClassAttribute("node", "stroke.size", "Stroke Size", 2.0, ATTR_FIXED);
  ...
  CAttribute weightAttr("weight", "Weight", 2.0, ATTR_FIXED);
                                  ...
                                  int p_y) //绘制圆圈+箭头
{
  ...
  if (data != -1) {
    ...
    pItem->setPos(QPointF(x + 5, y + 5));        //位置
    QFont font = pItem->font();
    font.setPixelSize(12); // 像素大小
    pItem->setFont(font);
    // pItem->setZValue(3);	//文字在最上层
    addItem(pItem);
  } else {
    // QGraphicsEllipseItem *rectItem = new QGraphicsEllipseItem;//绘制圆形
    QGraphicsRectItem *rectItem = new QGraphicsRectItem; //绘制矩形
                                   ...
                                   int p_y) //绘制圆圈+箭头
{
  ...
  rectItem->setPen(pen);
  rectItem->setBrush(QBrush(QColor(100, 149, 237))); //填充颜色
  rectItem->setRect(QRectF(x - 2, y, 30 + 4, 30));
  addItem(rectItem);
  ...
  int digit = 0;
  for (int i = 1, num1 = data1, shuzu = 0; data1 / i; i = (i * 10)) {
    num1 = num1 / 10;
    digit++; //循环进行一次,位数加一
  }
  pItem->setPos(QPointF(x - 4 * digit + 4, y + 5)); //位置
  QFont font = pItem->font();
  font.setPixelSize(12); // 像素大小
  pItem->setFont(font);
  // pItem->setZValue(3);	//文字在最上层
...
void CNodeEditorScene::drawCircleSearch(int x, int y, int data, int p_x,
                                        int p_y) //绘制圆圈+箭头
{
  QGraphicsLineItem *lineItem = new QGraphicsLineItem();
  QPen pen2 = lineItem->pen();       //画笔 绘制外框
...
void CNodeEditorScene::drawCircleSearch2(int x, int y, int data1, int data2,
                                         int p_x, int p_y) //绘制圆圈+箭头
{
  QGraphicsLineItem *lineItem = new QGraphicsLineItem();
  QPen pen2 = lineItem->pen();       //画笔 绘制外框
  pen2.setColor(QColor(255, 69, 0)); //外边框的颜色

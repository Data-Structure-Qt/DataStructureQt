...
CEditorScene::CEditorScene(QObject *parent)
    : Super(parent), m_doubleClick(false), m_dragInProgress(false),
      m_startDragItem(nullptr), m_draggedItem(nullptr),
      m_pimpl(new CEditorScene_p(this)), m_infoStatus(-1),
      // m_undoManager(new CSimpleUndoManager(*this)),
      m_undoManager(new CDiffUndoManager(*this)), m_menuTriggerItem(nullptr),
      m_needUpdateItems(true), m_labelsEnabled(true), m_labelsUpdate(false),
  ...
  setSceneRect(-500, -500, 1000, 1000); // -500
  ...
  // connections
  connect(this, &CEditorScene::selectionChanged, this,
          &CEditorScene::onSelectionChanged, Qt::DirectConnection);
  connect(this, &CEditorScene::focusItemChanged, this,
          &CEditorScene::onFocusItemChanged);
}
...
void CEditorScene::initialize() {
  ...
  // labels policy
  static CAttributeConstrainsEnum *labelsPolicy =
      new CAttributeConstrainsEnum();
...
void CEditorScene::enableGridSnap(bool on) { m_gridSnap = on; }
...
bool CEditorScene::setItemFactory(CItem *factoryItem,
...
CAttribute &CEditorScene::createClassAttribute(
    ...
    CAttributeConstrains *constrains, bool vis) {
  if (m_classAttributes[classId].contains(attrId)) {
    ...
    m_classAttributes[classId][attrId] =
        CAttribute(attrId, attrName, defaultValue, attrFlags);
...
void CEditorScene::setClassAttribute(const QByteArray &classId,
                                     const CAttribute &attr, bool vis) {
...
bool CEditorScene::removeClassAttribute(const QByteArray &classId,
                                        const QByteArray &attrId) {
...
void CEditorScene::setClassAttributeVisible(const QByteArray &classId,
                                            const QByteArray &attrId,
                                            bool vis) {
...
bool CEditorScene::isClassAttributeVisible(const QByteArray &classId,
                                           const QByteArray &attrId) const {
  return m_classAttributesVis[classId].contains(attrId);
}

QSet<QByteArray>
CEditorScene::getVisibleClassAttributes(const QByteArray &classId,
...
void CEditorScene::setVisibleClassAttributes(const QByteArray &classId,
...
const CAttribute CEditorScene::getClassAttribute(const QByteArray &classId,
                                                 const QByteArray &attrId,
                                                 bool inherited) const {
...
AttributesMap CEditorScene::getClassAttributes(const QByteArray &classId,
                                               bool inherited) const {
...
CAttributeConstrains *
CEditorScene::getClassAttributeConstrains(const QByteArray &classId,
                                          const QByteArray &attrId) const {
...
void CEditorScene::setClassAttributeConstrains(const QByteArray &classId,
                                               const QByteArray &attrId,
                                               CAttributeConstrains *cptr) {
  ClassAttrIndex index(classId, attrId);

  // do we need to clean up?
  // if (m_classAttributesConstrains.contains(index))
  //
  m_classAttributesConstrains[index];
...
void CEditorScene::copy() {
  ...
  mimeData->setData("base/selection", buffer);
...
void CEditorScene::pasteAt(const QPointF &anchor) {
  ...
  if (!mimeData)
    return;
  if (!mimeData->hasFormat("commonmoduels/selection"))
    return;

  deselectAll();

  // read items from the buffer
  QByteArray buffer = mimeData->data("commonmoduels/selection");
...
void CEditorScene::onItemDestroyed(CItem *citem) { Q_ASSERT(citem); }
...
void CEditorScene::onFocusItemChanged(QGraphicsItem *newFocusItem,
...
void CEditorScene::moveDrag(QGraphicsSceneMouseEvent *mouseEvent,
...
void CEditorScene::finishDrag(QGraphicsSceneMouseEvent *mouseEvent,
...
void CEditorScene::onMoving(QGraphicsSceneMouseEvent *mouseEvent,
...
void CEditorScene::onDragging(QGraphicsItem * /*dragItem*/,
                              const QSet<IInteractive *> & /*acceptedItems*/,
                              const QSet<IInteractive *> & /*rejectedItems*/) {
  updateCursorState();

  setInfoStatus(SIS_Drag);
}

void CEditorScene::onDropped(QGraphicsSceneMouseEvent *mouseEvent,
                             QGraphicsItem *dragItem) {
...
void CEditorScene::onLeftClick(QGraphicsSceneMouseEvent *mouseEvent,
...
void CEditorScene::onLeftDoubleClick(QGraphicsSceneMouseEvent *mouseEvent,
                                     QGraphicsItem *clickedItem) {
...
bool CEditorScene::onClickDrag(QGraphicsSceneMouseEvent *mouseEvent,
...
bool CEditorScene::onDoubleClickDrag(QGraphicsSceneMouseEvent *mouseEvent,
                                     const QPointF &clickPos) {
...
bool CEditorScene::doUpdateCursorState(Qt::KeyboardModifiers keys,
...
void CEditorScene::contextMenuEvent(
...
void CEditorScene::onActionDelete() {
  ...
  if (QMessageBox::question(NULL, tr("Delete Items"),
...
void CEditorScene::onActionSelectAll() { selectAll(); }

// label edit

void CEditorScene::onActionEditLabel(CItem *item, int data) {
  setInfoStatus(SIS_Edit_Label);
  setSceneCursor(Qt::IBeamCursor);

  m_pimpl->m_labelEditor.settext(item, data);
...
void CEditorScene::beginSelection() { blockSignals(true); }
...
QList<QGraphicsItem *>
CEditorScene::createSelectedList(const CItemsEvaluator &eval) const {
...
QObject *CEditorScene::createActions() { return new CEditorSceneActions(this); }

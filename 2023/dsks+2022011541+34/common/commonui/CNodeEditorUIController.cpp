...
void CNodeEditorUIController::createMenus() {
  ...
  QString::fromLocal8Bit("撤销")); undoAction->setStatusTip(tr("Undo latest
  action")); undoAction->setShortcut(QKeySequence::Undo); connect(undoAction,
  &QAction::triggered, this, &CNodeEditorUIController::undo);
  ...
  QString::fromLocal8Bit("重做")); redoAction->setStatusTip(tr("Redo latest
  action")); redoAction->setShortcut(QKeySequence::Redo); connect(redoAction,
  &QAction::triggered, this, &CNodeEditorUIController::redo);
  ...
  // QString::fromLocal8Bit("移动"));
  ////modeTransformAction->setToolTip(tr("Items transformation mode"));
  ////modeTransformAction->setStatusTip(tr("Transform selected nodes (scale &
  ///move)"));
  ...
  // QString::fromLocal8Bit("放缩"));
  ////modeFactorAction->setToolTip(tr("Positions transformation mode"));
  ////modeFactorAction->setStatusTip(tr("Scale position of selected nodes (move
  ///only)"));
  ...
  // QString::fromLocal8Bit("新建树"));
  ////modeTreeAction->setToolTip(tr("Create Binaray Tree by Input"));
  ////modeTreeAction->setStatusTip(tr("Scale position of selected nodes (move
  ///only)"));
  ...
  // add view menu
  m_viewMenu = new QMenu(QString::fromLocal8Bit("视图"));
  ...
  QString::fromLocal8Bit("适应窗口")); fitZoomAction->setStatusTip(tr("Zoom to
  ...
  QString::fromLocal8Bit("最佳比例")); fitZoomSelectedAction->setToolTip(tr("Fit
  selected items to view")); fitZoomSelectedAction->setStatusTip(tr("Zoom to fit
  selected items to view")); connect(fitZoomSelectedAction, &QAction::triggered,
  m_editorView, &CEditorView::fitSelectedToView);

  fitZoomBackAction = m_viewMenu->addAction(QIcon(":/Icons/ZoomFitBack"),
  QString::fromLocal8Bit("回退")); fitZoomBackAction->setStatusTip(tr("Zoom to
  previous state before last fit")); connect(fitZoomBackAction,
...
void CNodeEditorUIController::onClickGoToBeginButton() //走到起点
{
...
void CNodeEditorUIController::clickButtonDisabled() {
...
void CNodeEditorUIController::clickButtonEnabled() {
...
void CNodeEditorUIController::onClickPlayButton() {
  ...
  {
    playButtonFlag = 0;
    resetZoomAction->setText(
        QString::fromLocal8Bit("暂停")); //点击后进入此处变为待暂停
...
void CNodeEditorUIController::createPanels() {
  // default properties
  m_parent->createDockWindow("defaultsDock", QString::fromLocal8Bit("默认属性"),
                             Qt::LeftDockWidgetArea,
                             m_defaultsPanel =
                                 new CClassAttributesEditorUI(m_parent));

  m_defaultsPanel->setScene(m_editorScene);

  // properties
  m_parent->createDockWindow("propertyDock", QString::fromLocal8Bit("项目属性"),
                             Qt::RightDockWidgetArea,
                             m_propertiesPanel =
                                 new CNodeEdgePropertiesUI(m_parent));

  m_propertiesPanel->setScene(m_editorScene);

  // connections
  m_parent->createDockWindow(
      "connectionsDock", QString::fromLocal8Bit("拓扑关系"),
      Qt::LeftDockWidgetArea,
  ...
  m_parent->createDockWindow("linklistDock", QString::fromLocal8Bit("控制面板"),
                             Qt::LeftDockWidgetArea,
                             m_linklistPanel = new GraphUI(m_parent));
  m_linklistPanel->setScene(m_editorScene);

  //新加代码0712
  m_linklistPanel->setCNodeEditorUIController(this);
  ...
  m_linklistPanel->setCQuickHelpUI(m_quickHelpPanel);
...
void CNodeEditorUIController::onSceneChanged() {
  ...
  m_statusLabel->setText(
      QString::fromLocal8Bit("节点数") + tr(": %1 | ").arg(nodes.size()) +
      QString::fromLocal8Bit("边数") + tr(": %1").arg(edges.size()));
...
void CNodeEditorUIController::onNewDocumentCreated() {
  ...
  m_editorScene->createClassAttribute("", "comment", "Comment", QString(),
                                      ATTR_NONE);

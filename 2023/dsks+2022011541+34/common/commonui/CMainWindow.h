...
struct CDocumentFormat {
  ...
  QStringList suffixes; // supported suffixes like: png xpm jpg (first one
...
public:
  ...
  QDockWidget *createDockWindow(const QString &name, const QString &title,
  ...
  QList<CDocument> getRegisteredDocumentTypes() const {
...
protected:
  ...
  virtual bool getDocFormatFromName(const QString &normalizedName,
                                    const CDocument **doc,
                                    const CDocumentFormat **format,
                                    QString *suffix);

  virtual void onSaveDocumentDialog(QString &title, QString &filter) {}
  virtual bool doSaveDocument(const QString &fileName,

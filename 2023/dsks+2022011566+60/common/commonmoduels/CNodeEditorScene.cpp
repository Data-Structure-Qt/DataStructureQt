...
void CNodeEditorScene::set(int x, int y) {
	orgin.setX(x);
	orgin.setY(y);
}

void CNodeEditorScene::insertnode(int data)  //最上面的
{
	QPointF next(50, 0);
	if (data != -666) {
		qDebug("%d", data);
...
void CNodeEditorScene::drawText(int location)
{
	...
	pItem->setPlainText(QString::fromLocal8Bit("关键字排序后序列"));  // 纯文本
	pItem->setDefaultTextColor(QColor(0, 0, 0));  // 文本色
	pItem->setPos(QPointF(-100, 400));
...
{
	QRectF Rect(-10000, -10000, 10000, 10000);//删除-1000 -1000 1000 1000 方块内的所有图形

...
void BinaryTreeUI::playButton()
{
	...
	for
		(timestamp;
			timestamp<=onlytotaltime&& endplaybuttonflag!=1;timestamp++)
	{
		m_scene->clearScreen();//清屏
			for (int j = 0; j <= frametime[timestamp]; j++)
			{
...
void BinaryTreeUI::inOrderPrint_search(CBiTreeNode* root, double x,double y, double wideSize, double highSize,double p_x, double p_y, int data,int frame) 
{
	...
	wideSize = wideSize / 2;//每次将单位值除以二，绘制在根节点的左右
		...
		{
			inOrderPrint_search(root->left, x -
				wideSize, y + highSize, wideSize, highSize, x,
				y, data, 1);
		}
	...
	double p_x, double p_y)//递归展示函数
{
	...
	if (root->right != NULL)
	{
		double angle = Branch_angle -
			Change_angle + 2;//顺时针角度
			double radian = angle * 3.14159265 / 180; //计算弧度
			double Branch_length_x = Branch_length * cos(radian);//位移
			double Branch_length_y = Branch_length * sin(radian);//y位移
...
int BinaryTreeUI::getnable()
{
	QString data = ui->BinaryTreeDataEdit->text();
	list = data.split(",");
	int count = list.size(); // 获取list中数据的个数
	QString nable = ui->BinaryTreeDataEdit_2->text();
	bool flag;
	int number = nable.toInt(&flag);
	if (!flag)
	{
		// 处理转换失败的情况，例如返回一个默认值
		return -1; // 或者其他默认值
...
void BinaryTreeUI::on_binarytree_create_btn_clicked_1(int number)
{
	m_scene->clearScreen();//清屏
	avlTree = new CBiTree();//新建树
	root = avlTree->getRoot();//新建节点
...
void BinaryTreeUI::on_binarytree_create_btn_clicked_2(int number)
{
	m_scene->clearScreen();//清屏
	avlTree = new CBiTree();//新建树
	root = avlTree->getRoot();//新建节点
	...
	string LasOrd;
	PostOrderBiTree(root, LasOrd);
	temp =  QString::fromStdString(LasOrd);
	m_scene->printInOrd(0, -50, temp);
	for (int j = 0; j <= frametime[onlytotaltime]; j++)
	{
		animationDraw(onlytotaltime, j);
	}
...
//创建一个前m个元素的树
void BinaryTreeUI::str_create()
{
	int m = getnable();
	for (int i = 0; i <= m; i++)
	{
		on_binarytree_create_btn_clicked_1(i);
		sleepTime(2000); // 暂停两秒钟
		m_scene->clearScreen(); // 清屏
	}
	on_binarytree_create_btn_clicked_2(m);
}

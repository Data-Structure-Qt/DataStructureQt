	...
	if (root == NULL) {
		...
		while (!q.empty()) {
			...
			if (current->left == NULL) {
				...
				heapifydown(root, current->left);  // 插入后调整为最大堆
				break;  // 节点插入，退出循环
			}
			else {
				q.push(current->left);
			...
			if (current->right == NULL) {
				...
				heapifydown(root, current->right);  // 插入后调整为最大堆
				break;  // 节点插入，退出循环
			}
			else {
...
void CBiTree::heapifydown(CBiTreeNode* root, CBiTreeNode* insertedNode)
{
	...
	while (!q.empty()) {
		...
		//取出队列中位于同一行的结点放到cache中
			...
			for (int i = 0; i< cache.size(); i++)
			{
				...
				//遍历同一行的结点按照它们在中序序列的位置修改在line中的位置
					if (p)
					{
						char inStr[10];
						itoa(p->key, inStr, 10);

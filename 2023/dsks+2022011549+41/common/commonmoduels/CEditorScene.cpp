...
void CEditorScene::initialize()
{
	...
	if (labelsPolicy->ids.isEmpty()) {
		...
		labelsPolicy->ids << AlwaysOff << Auto << AlwaysOn;
	}
	createClassAttribute(class_scene, attr_labels_policy, "Labels Policy", AlwaysOn, ATTR_FIXED, labelsPolicy);
}
...
void CEditorScene::copy()
{
	...
	mimeData->setData("commonmoduels/selection", buffer);
	...
	tempScene->crop();
	QImage image(tempScene->sceneRect().size().toSize(),

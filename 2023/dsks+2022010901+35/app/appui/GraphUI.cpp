...
struct Queue {
	int count = 0;
	const int Y = 215; // 每个元素的y
	const int X[11] = { -350, -280, -210, -140, -70, 0, 70, 140, 210, 280, 350 }; // 每个元素的x
	...
	// CNode* que[20];
	QGraphicsRectItem *queue_square[20]; // 正放形
	QGraphicsTextItem *queue_label[20]; // id
	CNodeEditorScene *m_scene; // 指向场景对象的指针

	QGraphicsTextItem *pItem; // 文字“队头”
	QGraphicsTextItem *pItem1; // 文字“队尾”
	QGraphicsLineItem *lineItem; // 上线条
	QGraphicsLineItem *lineItem1; // 下线条


	void create(CNodeEditorScene *scene) { // 创建框架（文字、线条）
		...
		pItem->setPlainText(QString::fromLocal8Bit("队头"));  // 纯文本
		pItem->setDefaultTextColor(QColor(0, 0, 0));  // 文本色
		pItem->setPos(QPointF(-365, 170));//位置
	...
	void push(const QString &id) {
		...
		/*
		CNode *push_node = m_scene->createNewNode(QPointF(X[count], Y)); // 之前忘记++了
		push_node->setId(id + "."); // 貌似id不能重复
		que[count]->setLabelSize(20); // 设置文字大小为20
		...
		*/
		...
		queue_square[count] = square; // 保存到结构体中
	...
	void pop() {
		...
		if (count == 0) {
			...
			for (int i = 1; i < count; ++i) {
				que[i - 1]->setId(que[i]->getId()); // id向前平移
			}
			m_scene->removeItem(que[count - 1]); // 再删除尾部节点
			delete que[--count];
			*/
			...
			for (int i = 1; i < count; ++i) {
				queue_label[i - 1]->setPlainText(queue_label[i]->toPlainText()); // 向前平移label
			}
			m_scene->removeItem(queue_label[count - 1]); // 删除尾部的label和square
...
void GraphUI::on_no35_btn_clicked()
{
	...
	node_a->setSelected(true); // 设置a节点为被选中的状态

	GraphUI::on_bfs_btn_clicked(); // 直接执行bfs函数
...
void GraphUI::on_bfs_btn_clicked()
{
	...
	for (auto node : nodes)
	{
		...
		node->setLabelSize(20); // 设置文字大小为20
		node->showLabel(true);
		//显示标签 
	}
	onlytotaltime = 0;
	...
	// que.m_scene = m_scene; // 设定场景
	que.create(m_scene); // 绘制自定义队列的线段和文字
			...
			while (!bfsQueue.empty()) {
				...
				while (node) {
					if (!Mvisited[node->adjointVertex]) {
						...
						que.push(QString::fromStdString(g.adjointList[node->adjointVertex].data)); // 加入到自己定义的队列中
					}
					node = node->next;
				}
				que.pop(); // 从自己定义的队列中弹出
...
//v是G中的某个顶点，w是v的邻接顶点 
//返回v的(相对于w的)下一个邻接顶点。若w是v的最后一个邻接点，则返回
...
void GraphUI::on_prim_btn_clicked()  //注意：prim算法需要设置边的权值 
{
	m_nodeEditorUIController->clickButtonDisabled();
	AMLGraph g;
	CreateGraph(&g);
...
int GraphUI::topoCreate(topoNode topoadj[], int n, int m)//邻接表建表函数，n
{
...
{
	QTime dieTime = QTime::currentTime().addMSecs(time);//获取自定义休眠时
		while (QTime::currentTime() < dieTime)
		{
			QCoreApplication::processEvents(QEventLoop::AllEvents, 100); //开启
		}
}

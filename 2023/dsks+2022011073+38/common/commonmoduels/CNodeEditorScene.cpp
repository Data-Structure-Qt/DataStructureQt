...
void CNodeEditorScene::initialize()
{
	...
	// default node attr
	CAttribute nodeAttr("color", "Color", QColor("#3399ff"), ATTR_FIXED);//初始为蓝色
	setClassAttribute("node", nodeAttr);

	CAttribute shapeAttr("shape", "Shape", "disc", ATTR_FIXED);
	setClassAttribute("node", shapeAttr);

	createClassAttribute("node", "size", "Size", QSizeF(70.0, 70.0), ATTR_MAPPED | ATTR_FIXED);
...
CNode* CNodeEditorScene::createNewNode(const QPointF& pos)
{
	...
	addItem(node);
	node->setPos(pos);
	return node;// TODO: Add your command handler code here
}
...
CEdge* CNodeEditorScene::createNewConnection(CNode* startNode, CNode* endNode)
{
	...
	return edge;// TODO: Add your command handler code her
...
void CNodeEditorScene::drawCircleSearch(int x, int y, int data, int p_x, int p_y)//绘制圆圈+箭头
{
	QGraphicsLineItem *lineItem = new QGraphicsLineItem();
	QPen pen2 = lineItem->pen();//画笔 绘制外框
...
void CNodeEditorScene::drawCircleSearch2(int x, int y, int data1, int data2, int p_x, int p_y)//绘制圆圈+箭头
{
	QGraphicsLineItem *lineItem = new QGraphicsLineItem();
	QPen pen2 = lineItem->pen();//画笔 绘制外框
	pen2.setColor(QColor(255, 69, 0));//外边框的颜色

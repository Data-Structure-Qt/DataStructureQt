...
void GraphUI::createGraph(MGraph &g) {
	...
	int nodei = 0; //计数器
	QList<CNode*> selectedNodes = m_scene->getSelectedNodes();//第一个节点
	//存入数组
		if (selectedNodes.isEmpty() == false) {
			g.adjointList[nodei].data = selectedNodes.at(0)->getId().toStdString();//转
			//为 string 类型
	...
	for (auto node : nodes)//其余进入数组
	{
		if (node->getId().toStdString() !=
			...
			g.adjointList[nodei].data = node->getId().toStdString();//转为 string 
			//类型
				g.adjointList[nodei].firstEdage = NULL;
			nodei++;
		}
...
//求割点的算法
int  GraphUI::locatevex(ALGraph G, char v)
{
	int i;
...
void GraphUI::createGraph(ALGraph *G) {
	if (G == NULL) {
		qDebug() << "G is NULL";
		return;
	}
	...
	{
		//转为 string 类型
		G->vertices[nodei].data = node->getId().toStdString()[0]; // 只取第一个字符作为顶点名称
		G->vertices[nodei].firstarc = NULL;
		nodei++;
	}
	for (auto edge : edges)
	{
		...
		int i1 = locatevex(*G, v1[0]); // 只取第一个字符作为顶点名称
		int i2 = locatevex(*G, v2[0]); // 只取第一个字符作为顶点名称
		ArcNode *p = new ArcNode;
		if (p != NULL) {
			p->adjvex = i2;
			p->nextarc = G->vertices[i1].firstarc;
			G->vertices[i1].firstarc = p;
		}
		ArcNode *q = new ArcNode;
		if (q != NULL) {
			q->adjvex = i1;
			q->nextarc = G->vertices[i2].firstarc;
			G->vertices[i2].firstarc = q;
		}
	}
}
...
//获取图的关节点（割点）
int visit[MAX_VERTEX_NUM], low[MAX_VERTEX_NUM], c;
void GraphUI::DFSArticul(ALGraph G, int v)
{
	int min = visit[v] = c++;  //v是第count个访问的顶点
	ArcNode *p;
	for (p = G.vertices[v].firstarc; p != NULL; p = p->nextarc)      //对v的每个邻接顶点检查 
	{
		int w = p->adjvex;   //w为v的邻接顶点
		if (visit[w] == 0)   //w未曾访问是v的孩子 
		{
			...
			if (low[w] >= visit[v])
			{
				NodeChangeColor(QString::number(G.vertices[v].data)); // 改变割点的颜色
			}
		}
...
void GraphUI::findArticul(ALGraph G)
{
	int i, v;
	ArcNode *p;
	c = 1;
...
void GraphUI::PrintGraph(AMLGraph G)
{
	...
	for (int i = 0; i < G.vexnum; ++i)
	{
		...
		while (p != NULL)
		{
			...
			{
				//cout << G.adjmulist[p->ivex].data << "---" << p->weight << "--
				//-" << G.adjmulist[p->jvex].data << endl;
					p = p->ilink;
			}
			else
			{
				//cout << G.adjmulist[p->jvex].data << "---" << p->weight << "-
				//--" << G.adjmulist[p->ivex].data << endl;
					p = p->jlink;
			}
		}
	}

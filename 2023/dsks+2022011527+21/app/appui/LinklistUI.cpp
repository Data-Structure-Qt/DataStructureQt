...
void LinklistUI::runAlgorithm()
{
	//从文本框获取字符串
	QString s = ui->lineEditS->text();
	...
	if (maxLength > 0)
	{
		resultText = s.mid(endIndexS - maxLength + 1, maxLength);
		s.replace(endIndexS - maxLength + 1, maxLength, "<font color='red'>" + resultText + "</font>");
	}

	if (maxLength > 0)
	{
		resultText = t.mid(endIndexT - maxLength + 1, maxLength);
		t.replace(endIndexT - maxLength + 1, maxLength, "<font color='blue'>" + resultText + "</font>");
	}
	//输出结果
	ui->textBrowserS->clear();
	ui->textBrowserT->clear();

	ui->textBrowserS->setFont(QFont("Arial", 20));
	ui->textBrowserT->setFont(QFont("Arial", 20));

	ui->textBrowserS->append(s);
	ui->textBrowserT->append(t);
}

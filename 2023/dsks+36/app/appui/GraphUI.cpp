...
GraphUI::~GraphUI() { maxPathLength = 0; }
...
void GraphUI::on_longestPath_btn_clicked()
{
	// 获取屏幕目前的边和点
	QList<CNode*> nodes = m_scene->getItems<CNode>();
	QList<CEdge*> edges = m_scene->getItems<CEdge>();
	...
	createGraph(g); // 依据屏幕边与节点创建图
	DFSTraverse(g);
	vector<CNode*> longest_path;
	for (auto node : nodes) {
		//if (node->getId() ==  maxPathNodes[i]) {
			//NodeChangeColorWithoutSuspand(node->getId());
		//}
		int flag = 0;
		for (auto s : maxPathNodes) {
			if (s == node->getId()) {
		...
		for (auto edge : edges){
			int t = 0;
			for (int i=0; i < maxPathNodes.length()-1;i++) {
				if (QString(edge->firstNode()->getId()) == maxPathNodes[i] && QString(edge->lastNode()->getId()) == maxPathNodes[i + 1]) {
					edge->setAttribute("color", "#FF0000");
					t = 1;
				}
			}
			if (t == 0) {
				edge->setAttribute("color", "#3399ff");
			}
	   }
...
void GraphUI::DFSTraverse(MGraph g) {
	...
	for (int i = 0; i < g.numVertex; i++) {
		if (!Mvisited[i]) {
			...
			// 输出或使用最长路径信息
			qDebug() << "Longest Path: " << maxPathNodes;

			
			// QString firstNodeID = m_nodeFactory->getId();
			// QString lastNodeID = m_nodeFactory->getId();

			// EdgeChangeColor(firstNodeID, lastNodeID);
			// EdgeChangeColorWithoutSuspand(firstNodeID, lastNodeID);
		}
		else {

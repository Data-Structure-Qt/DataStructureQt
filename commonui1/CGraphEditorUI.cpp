...
#include "CGraphEditorUI.h"
#include "ui_CGraphEditorUI.h"
#include "../base/CNodeEditorScene.h"
#include <QFile>
#include <QTextStream>
#include <base/CNodeEditorScene.h>
#include <base/CEdge.h>
#include <base/CNode.h>
#include <QString>
#include <QDebug>
#include <QColor>
#include <QTime>
CGraphEditorUI::CGraphEditorUI(QWidget *parent) :
	QWidget(parent),
	ui(new Ui::CGraphEditorUI),
	m_scene(NULL)
{
	ui->setupUi(this);
}
void CGraphEditorUI::onSceneDetached(CEditorScene* scene) {
	scene->disconnect(this);
}
void CGraphEditorUI::setScene(CNodeEditorScene* scene) {
	if (m_scene)
...
void CGraphEditorUI::setCQuickHelpUI(CQuickHelpUI * scene)
{
	m_quickHelp = scene;
	setEnabled(m_quickHelp);
}
void CGraphEditorUI::onSceneAttached(CEditorScene* scene) {}
CGraphEditorUI::~CGraphEditorUI() {}
void
CGraphEditorUI::setCNodeEditorUIController(CNodeEditorUIController*
	nodeEditorUIController)
{
	m_nodeEditorUIController = nodeEditorUIController;
	setEnabled(m_nodeEditorUIController);
}
void CGraphEditorUI::on_bfs_btn_clicked()
{
...
void CGraphEditorUI::on_dfs_btn_clicked()
{
	m_nodeEditorUIController->clickButtonDisabled();
	//获取屏幕目前的边和点
	QList<CNode*> nodes = m_scene->getItems<CNode>();
	...
	for (auto node : nodes)//显示节点标签
	{
		node->setSize(70, 70);
		node->setLabelText(node->getId());
		node->setLabelPosition(node->x() / 4096 - 10, node->y() / 4096 - 10);//设置标签位置
...
void CGraphEditorUI::on_reset_btn_clicked()
{
	QList<CNode*> nodes = m_scene->getItems<CNode>();
	QList<CEdge*> edges = m_scene->getItems<CEdge>();
...
void CGraphEditorUI::createGraph(MGraph &g) {
	QList<CNode*> nodes = m_scene->getItems<CNode>();
	QList<CEdge*> edges = m_scene->getItems<CEdge>();
	g.numVertex = nodes.count();//传入点数量
	g.numEdge = edges.count();//传入边数量
	int nodei = 0; //计数器
	QList<CNode*> selectedNodes = m_scene->getSelectedNodes();//第一个节存入数组
...
int CGraphEditorUI::getPosition(MGraph g, string value)
{
...
void CGraphEditorUI::BFSTraverse(MGraph g) {
	CGraphVertexNode *node;
	queue<int> bfsQueue;
			...
			while (!bfsQueue.empty()) {
				...
				while (node) {
					if (!Mvisited[node->adjointVertex]) {
						...
						// 记 录 时 间 +1 EdgeChangeColor 节 点ID = QString::fromStdString(g.adjointList[i].data)
							animationRecord(2,
						...
						// 记 录 时 间 +1 NodeChangeColor 节 ID = QString::fromStdString(g.adjointList[i].data)
							animationRecord(1,
...
//改变节点的颜色
void CGraphEditorUI::NodeChangeColor(QString nodeID)
{
	QList<CNode*> nodes = m_scene->getItems<CNode>();
...
void CGraphEditorUI::NodeChangeColorWithoutSuspand(QString nodeID)
{
	QList<CNode*> nodes = m_scene->getItems<CNode>();
	for (auto m_node : nodes) {
		if (m_node->getId() == nodeID) {
...
//改变线段的颜色
void CGraphEditorUI::EdgeChangeColor(QString firstNodeID, QString
	lastNodeID)
{
	QList<CEdge*> edges = m_scene->getItems<CEdge>();
...
//改变线段的颜色
void CGraphEditorUI::EdgeChangeColorWithoutSuspand(QString firstNodeID,
	QString
	lastNodeID)
{
	QList<CEdge*> edges = m_scene->getItems<CEdge>();
...
void CGraphEditorUI::DFS(MGraph g, int index) {
	CGraphVertexNode *node;
	Mvisited[index] = true;
	//输出节点
	...
	animationRecord(1,
		QString::fromStdString(g.adjointList[index].data), NULL);
	...
	while (node) {
		...
		if (!Mvisited[node->adjointVertex]) {
			...
			animationRecord(2,
				QString::fromStdString(g.adjointList[index].data),
...
void CGraphEditorUI::DFSTraverse(MGraph g) {
	//节点初始化 未访问
	for (int i = 0; i < g.numVertex; i++) {
		Mvisited[i] = false;
	}
...
void CGraphEditorUI::destroyGraph(MGraph &g) {
...
void CGraphEditorUI::on_dijkstra_btn_clicked()
{
	m_nodeEditorUIController->clickButtonDisabled();
	// 数组都从下标 1 开始
...
void CGraphEditorUI::MDijkstra(int n, int v, int *dist, int *prev, int
	c[maxnum][maxnum])
{
...
void CGraphEditorUI::searchPath(int *prev, int v, int u)
{
	int que[maxnum];
	int tot = 1;
	que[tot] = u;
...
//prim 算法
int CGraphEditorUI::LocateVex(VexBox adj[], int num, string v) {
	for (int i = 0; i < num; i++) {
		if (adj[i].data == v) {
			return i;
		}
...
void CGraphEditorUI::CreateGraph(AMLGraph *G) {
	QList<CNode*> nodes = m_scene->getItems<CNode>();
	QList<CEdge*> edges = m_scene->getItems<CEdge>();
...
int CGraphEditorUI::FirstAdjVex(AMLGraph G, int v, int &wei) {
	int i = v;
...
int CGraphEditorUI::NextAdjVex(AMLGraph G, int v, int w, int &wei) {
	int i1 = v;
	int i2 = w;
...
void CGraphEditorUI::PrintGraph(AMLGraph G)
{
	EBox *p;
...
void CGraphEditorUI::MiniSpanTree_PRIM(AMLGraph G) {
	struct {
		int adjvex;
		int lowcost;
	}close[1010];
...
void CGraphEditorUI::on_prim_btn_clicked() //注意：prim 算法需要设置边的权值
{
	m_nodeEditorUIController->clickButtonDisabled();
	AMLGraph g;
	CreateGraph(&g);
...
//Kruskal 算法
int CGraphEditorUI::FindSet(vector<int> uset, int i) {
	return uset[i] == i ? i : FindSet(uset, uset[i]);
}
void CGraphEditorUI::MiniSpanTree_Kruskal(AMLGraph G) {
...
void CGraphEditorUI::on_kruskal_btn_clicked()
{
	m_nodeEditorUIController->clickButtonDisabled();
	AMLGraph g;
	CreateGraph(&g);
...
int CGraphEditorUI::topoCreate(topoNode topoadj[], int n, int m)//邻接表建表函数，n 代表定点数，m 代表边数
{
	topoNode *p;
	for (int i = 1; i <= n; i++)
	{
...
void CGraphEditorUI::topoPrint(int n)//邻接表打印函数
{
	int i;
	topoNode *p;
	for (i = 1; i <= n; i++)
...
void CGraphEditorUI::topoSort(topoNode topoadj[], int n)
{
	int toponode;
	topoNode *p;
...
void CGraphEditorUI::on_topologicalSorting_btn_clicked()
{
	m_nodeEditorUIController->clickButtonDisabled();
	int n;
	int m;
...
void CGraphEditorUI::DFSNonRecursive(MGraph g, int index) {
	CGraphVertexNode* node;
	Mvisited[index] = true;
	//stack <int> mystack;
	//stack <int> mystack2;
	...
	while (stack.size() != 0) {
		int v = stack.top();
		bool bFind = false;//判断是否存在链接元素未处理
		//标记为已访问
		Mvisited[v] = true;
		//遍历顶点V的相邻顶点
		node = g.adjointList[v].firstEdage;
		while (node) {
			if (!Mvisited[node->adjointVertex]) {
				stack.push(node->adjointVertex);
				bFind = true;
			}
			node = node->next;
		}
		if (bFind == false)//如果没有找到节点，视作后续已没有需要处理的节点，视作结束
		{
			if (mystack2.size() > 0 && mystack2.top() == v)
			{
				mystack2.pop();
			}
			else if (mystack2.size() >0)
			{
				...
				EdgeChangeColor(QString::fromStdString(g.adjointList[mystack2.top()].data),
					QString::fromStdString(g.adjointList[v].data));
				m_quickHelp->dfs_code(2);
				animationRecord(2,
					QString::fromStdString(g.adjointList[mystack2.top()].data),
					QString::fromStdString(g.adjointList[v].data), 1, 2);
			}
			stack.pop();//获取栈顶元素
			//输出节点
			NodeChangeColor(QString::fromStdString(g.adjointList[v].data)); //节点变色
			animationRecord(1,
				QString::fromStdString(g.adjointList[v].data), NULL);
		}
		else
		{
			mystack2.push(v);
...
void CGraphEditorUI::DFSNonRecursiveTraverse(MGraph g)
{
	//节点初始化 未访问
	for (int i = 0; i < g.numVertex; i++) {
		Mvisited[i] = false;
...
void CGraphEditorUI::on_Non_topologicalSorting_btn_clicked()
{
	m_nodeEditorUIController->clickButtonDisabled();
	int n;
	int m;
	...
	MGraph g;
	createGraph(g);
	DFSNonRecursiveTraverse(g);
	destroyGraph(g);
	m_scene->deselectAll();//取消节点全选
	m_nodeEditorUIController->clickButtonEnabled();
}
void CGraphEditorUI::goToBeginButton()
{
...
void CGraphEditorUI::stepBackButton()
{
	endplaybuttonflag = 1;
	on_reset_btn_clicked();
	if (timestamp > 1)
...
void CGraphEditorUI::playButton()
{
	endplaybuttonflag = 0;
...
void CGraphEditorUI::suspendButton()
{
	endplaybuttonflag = 1;
}
void CGraphEditorUI::stepForwardButton()
{
	endplaybuttonflag = 1;
...
void CGraphEditorUI::goToEndButton()
{
	endplaybuttonflag = 1;
	//m_scene->clearScreen();//清屏
...
void CGraphEditorUI::animationDraw(int totaltime)
{
...
void CGraphEditorUI::animationDrawWithoutSuspand(int totaltime)
{
	if (animation[totaltime].functionid == 1) {
...
void CGraphEditorUI::sleepTime(int time)
{
...
void CGraphEditorUI::setCEditorView(CEditorView* editorView)
{
	m_editorView = editorView;
	setEnabled(m_editorView);
}
//totaltime 总运行时间 functionid 1 节点，2 边
void CGraphEditorUI::animationRecord(int functionid, QString para1, QString
...
void CGraphEditorUI::animationview(int currenttime)
{

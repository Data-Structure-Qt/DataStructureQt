...
void CSceneMenuUIController::fillMenu(QMenu &menu, CEditorScene *scene, QGraphicsItem *triggerItem, QGraphicsSceneContextMenuEvent* /*contextMenuEvent*/)
{
	...
	// add default actions 更改节点权值 1219
	//QAction *changeValueAction = menu.addAction(tr("Change Value..."), parent(), SLOT(changeItemValue()));
	//changeValueAction->setEnabled((nodesCount + edgesCount) == 1);
    ...
    QAction *dfsAction = menu.addAction(tr("DFS"), sceneActions, SLOT(onActionDFS()));
    dfsAction->setEnabled(nodesSelected);

    QAction *bfsAction = menu.addAction(tr("BFS"), sceneActions, SLOT(onActionBFS()));
    bfsAction->setEnabled(nodesSelected);

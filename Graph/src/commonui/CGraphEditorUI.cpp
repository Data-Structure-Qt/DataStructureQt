...
void CGraphEditorUI::setCNodeEditorUIController(CNodeEditorUIController* nodeEditorUIController)
{
...
void CGraphEditorUI::BFSTraverse(MGraph g) {
	...
	for (int i = 0; i < g.numVertex; i++) {
		...
		if (!Mvisited[i]) {
			...
			//记录 时间+1 NodeChangeColor  节点ID=QString::fromStdString(g.adjointList[i].data)
			...
			while (!bfsQueue.empty()) {
				...
				while (node) {
					if (!Mvisited[node->adjointVertex]) {
						...
						//记录 时间+1 EdgeChangeColor  节点ID=QString::fromStdString(g.adjointList[i].data)
...
//改变线段的颜色
void CGraphEditorUI::EdgeChangeColorWithoutSuspand(QString firstNodeID, QString
	lastNodeID)
{
	QList<CEdge*> edges = m_scene->getItems<CEdge>();
	for (auto m_edge : edges)
...
void CGraphEditorUI::MDijkstra(int n, int v, int *dist, int *prev, int c[maxnum][maxnum])
{
...
void CGraphEditorUI::on_prim_btn_clicked()  //注意：prim算法需要设置边的权值
{
	m_nodeEditorUIController->clickButtonDisabled();
	AMLGraph g;
	CreateGraph(&g);
...
void CGraphEditorUI::MiniSpanTree_Kruskal(AMLGraph G) {
	...
	for (int i = 0; i < edg.size(); i++) {
		...
		if (e1 != e2) {
			...
			for (auto edge : edges)
			{	//
				if (edge->getId() == QString::number(edg[i].weight) && edge->getAttribute("color") != "#FF0000")
				{
					EdgeChangeColor(edge->firstNode()->getId(),
						edge->lastNode()->getId());
					animationRecord( 2, edge->firstNode()->getId(),
...
int CGraphEditorUI::topoCreate(topoNode topoadj[], int n, int m)//邻接表建表函数，n代表定点数，m代表边数
{
...
//totaltime 总运行时间 functionid 1 节点，2边 
void CGraphEditorUI::animationRecord(int functionid, QString para1, QString para2,int codefunctionid,int codefunctionlineid)
{
	onlytotaltime++;
...
void CGraphEditorUI::animationRecord(int functionid, QString para1, QString para2)
{
	onlytotaltime++;
	int totaltime = onlytotaltime;

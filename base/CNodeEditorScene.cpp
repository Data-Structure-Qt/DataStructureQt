...
void CNodeEditorScene::drawCircle3(int x, int y, char data, int p_x, int p_y)
{
	QString neirong;//内容
	neirong += data;
	if (data == 'a')
	{
		QGraphicsLineItem* lineItem = new QGraphicsLineItem();
		QPen pena1 = lineItem->pen();//画笔 绘制外框
		pena1.setColor(QColor(100, 149, 237));//外边框的颜色
		pena1.setWidth(2);
		lineItem->setPen(pena1);
		lineItem->setLine(x, y, p_x - 150, p_y + 50);
		addItem(lineItem);

		lineItem = new QGraphicsLineItem();
		QPen pena2 = lineItem->pen();//画笔 绘制外框
		pena2.setColor(QColor(100, 149, 237));//外边框的颜色
		pena2.setWidth(2);
		lineItem->setPen(pena2);
		lineItem->setLine(x , y , p_x+1, p_y+50 );//c
		addItem(lineItem);

		lineItem = new QGraphicsLineItem();
		QPen pena3 = lineItem->pen();//画笔 绘制外框
		pena3.setColor(QColor(100, 149, 237));//外边框的颜色
		pena3.setWidth(2);
		lineItem->setPen(pena3);
		lineItem->setLine(x, y, p_x + 150, p_y + 50);//c
		addItem(lineItem);
		...
		rectItem->setPen(pen);
		rectItem->setBrush(QBrush(QColor(100, 149, 237)));//填充颜色
		rectItem->setRect(QRectF(x-15, y-15, 30, 30));
		addItem(rectItem);

		QGraphicsTextItem* pItem = new QGraphicsTextItem();
		pItem->setPlainText(neirong);  // 纯文本
		pItem->setDefaultTextColor(QColor(0, 0, 0));  // 文本色
		pItem->setPos(QPointF(x-7 , y-10));//位置
		QFont font = pItem->font();
		font.setPixelSize(12);  // 像素大小
		pItem->setFont(font);
		addItem(pItem);
	...
	else if (data == 'b')
	{
		...
		pena1.setWidth(2);
		lineItem->setPen(pena1);
		lineItem->setLine(x, y, p_x - 50, p_y + 50);
		addItem(lineItem);
		...
		pena3.setWidth(2);
		lineItem->setPen(pena3);
		lineItem->setLine(x, y, p_x + 50, p_y + 50);//c
		addItem(lineItem);
	...
	else if (data == 'c')
	{
		...
		pena1.setWidth(2);
		lineItem->setPen(pena1);
		lineItem->setLine(x, y, p_x +1, p_y + 50);
		addItem(lineItem);
	...
	else if (data == 'h')
	{
		...
		pena1.setWidth(2);
		lineItem->setPen(pena1);
		lineItem->setLine(x, y, p_x - 25, p_y + 50);
		addItem(lineItem);
		...
		pena2.setWidth(2);
		lineItem->setPen(pena2);
		lineItem->setLine(x, y, p_x + 25, p_y + 50);//c
		addItem(lineItem);
...
void CNodeEditorScene::NewLine(char data1,int x, int y,char data2, int p_x, int p_y)
{
	QString neirong1,neirong2;//内容
	neirong1 += data1;
	neirong2 += data2;
	QGraphicsLineItem* lineItem = new QGraphicsLineItem();
	QPen pena1 = lineItem->pen();//画笔 绘制外框
	pena1.setColor(QColor(0, 0, 0));//外边框的颜色
	pena1.setWidth(2);
	lineItem->setPen(pena1);
	lineItem->setLine(x, y, p_x, p_y);
	addItem(lineItem);
	QGraphicsEllipseItem* rectItem = new QGraphicsEllipseItem;//绘制圆形
	QPen pen = rectItem->pen();//画笔 绘制外框
	pen.setColor(Qt::green);//外边框的颜色
	rectItem->setPen(pen);
	rectItem->setBrush(QBrush(QColor(100, 149, 237)));//填充颜色
	rectItem->setRect(QRectF(p_x- 15, p_y - 15, 30, 30));
	addItem(rectItem);

	QGraphicsTextItem* pItem = new QGraphicsTextItem();
	pItem->setPlainText(neirong2);  // 纯文本
	pItem->setDefaultTextColor(QColor(0, 0, 0));  // 文本色
	pItem->setPos(QPointF(p_x - 7, p_y - 10));//位置
	QFont font = pItem->font();
	font.setPixelSize(12);  // 像素大小
	pItem->setFont(font);
	addItem(pItem);

	rectItem = new QGraphicsEllipseItem;//绘制圆形
	pen = rectItem->pen();//画笔 绘制外框
	pen.setColor(Qt::green);//外边框的颜色
	rectItem->setPen(pen);
	rectItem->setBrush(QBrush(QColor(100, 149, 237)));//填充颜色
	rectItem->setRect(QRectF(x - 15, y - 15, 30, 30));
	...
	pItem->setPlainText(neirong1);  // 纯文本
	pItem->setDefaultTextColor(QColor(0, 0, 0));  // 文本色
	pItem->setPos(QPointF(x - 7, y - 10));//位置
	font = pItem->font();
	font.setPixelSize(12);  // 像素大小
...
void CNodeEditorScene::BlackLine(char data1, int x, int y, char data2, int p_x, int p_y)
{
	QString neirong1, neirong2;//内容
	neirong1 += data1;
	neirong2 += data2;
	QGraphicsLineItem* lineItem = new QGraphicsLineItem();
	QPen pena1 = lineItem->pen();//画笔 绘制外框
	pena1.setColor(QColor(255, 0, 0));//外边框的颜色
	pena1.setWidth(2);
	lineItem->setPen(pena1);
	lineItem->setLine(x, y, p_x, p_y);
	addItem(lineItem);

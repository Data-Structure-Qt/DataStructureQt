...
void CEditorScene::pasteAt(const QPointF &anchor)
{
    ...
    if (!mimeData)
		return;
	if (!mimeData->hasFormat("base/selection"))
		return;

	deselectAll();

	// read items from the buffer
	QByteArray buffer = mimeData->data("base/selection");
...
//QGraphicsRectItem* CEditorScene::getRectItemAt(const QPointF& pos) const
//{
//	QGraphicsRectItem *hoverItem = rectitemAt(pos, QTransform());
//
//	return hoverItem;
//}

...
//    int H=height();
    ...
    while(!Q.isEmpty())
    {
        ...
        if(p->left!=nullptr)
        {
            ...
            My_lines[i-1].setP1(p->point+QPoint(-22,12));
            My_lines[i-1].setP2(p->left->point-QPoint(-22,10));
            Q.enqueue(p->left);
        }
        ...
        if(p->right!=nullptr)
        {
            ...
            My_lines[i-1].setP1(p->point+QPoint(22,20));
            My_lines[i-1].setP2(p->right->point-QPoint(22,10));
            Q.enqueue(p->right);
            h--;
        }
...
//查找
void BinaryTree::Find(QChar ch, BinTreeNode *Node)
{
    if(Node==nullptr)
        return;
    if(Node->data==ch)

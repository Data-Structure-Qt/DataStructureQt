...
_again:
	...
	QString newId = QInputDialog::getText(0, 
		QString::fromLocal8Bit("改变节点名称"),
		QString::fromLocal8Bit("指定新节点名称"),
		QLineEdit::Normal, 
		editId);
	...
	for (auto item : items)
	{
		...
		if (node->getId() == newId)
		{
			...
			QString autoId = QString(QString::fromLocal8Bit("指定新节点名称").arg(nextFreeId));

			int r = QMessageBox::warning(0,
				QString::fromLocal8Bit("警告：Id已经被使用"),
				QString::fromLocal8Bit("Id %1 已经在其他节点使用").arg(newId),
				autoId,
				QString::fromLocal8Bit("交换节点Id"),
				QString::fromLocal8Bit("继续编辑"), 0, 2);
			if (r == 2)
			{
				editId = newId;
				goto _again;
